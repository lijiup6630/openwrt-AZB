...
function full_reset(){
	...
	if [ -f /usr/bin/wifi_api ]; then
		print_logs "NVRAM status before factory: $(df /nvram)"
		print_logs "SDB status before factory: $(ls -ls /nvram/itstore/wave_trusted_db.ini)"
		/usr/bin/wifi_api wifi_factorySecureDB
		print_logs "SDB status after factory: $(ls -ls /nvram/itstore/wave_trusted_db.ini)"
		print_logs "NVRAM status after factory: $(df /nvram)"
	fi
