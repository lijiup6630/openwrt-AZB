...
function get_index_from_map_file(){
	...
	[ -n "$line_number" ] && radio_map_file_content=$(echo "$radio_map_file_content" | sed "${line_number}d")
...
function validate_radio_map_file(){
	...
	# copy radio_map_file
	radio_map_file_name=$DEFAULT_DB_PATH/radio_map_file_$(lspci -n | grep -c -E "$LTQ_CODE_WAVE600|$LTQ_CODE_WAVE600D2|$LTQ_CODE_WAVE600D2B")_cards
...
function reset_radio(){
	...
	local greylist_file=`uci show wireless | grep greylist_file | grep
	$radio | cut -d"=" -f2`
	...
	if [ -f /usr/bin/wifi_api ]; then
		print_logs "NVRAM status before radio factory: $(df /nvram)"
		print_logs "SDB status before radio factory: $(ls -ls /nvram/itstore/wave_trusted_db.ini)"
		/usr/bin/wifi_api wifi_factorySecureDB radio "$iface"
		print_logs "SDB status after radio factory: $(ls -ls /nvram/itstore/wave_trusted_db.ini)"
		print_logs "NVRAM status after radio factory: $(df /nvram)"
	fi
...
function reset_vap(){
	...
	if [ -f /usr/bin/wifi_api ]; then
		print_logs "NVRAM status before vap factory: $(df /nvram)"
		print_logs "SDB status before vap factory: $(ls -ls /nvram/itstore/wave_trusted_db.ini)"
		/usr/bin/wifi_api wifi_factorySecureDB vap $1
		print_logs "SDB status after vap factory: $(ls -ls /nvram/itstore/wave_trusted_db.ini)"
		print_logs "NVRAM status after vap factory: $(df /nvram)"
	fi
