...
drv_mac80211_init_device_config() {
	...
	config_add_int unsolicited_frame_support unsolicited_frame_duration acs_fils
...
mac80211_hostapd_setup_base() {
	...
	if [ "$band" = "5GHz" ]; then
		...
		case "$htmode" in
			VHT20|VHT40*)
				vht_oper_chwidth=0
				if [ "$channel" -ge 36 ] && [ "$channel" -le 64 ]; then
					op_class=115
				fi
				if [ "$channel" -ge 100 ] && [ "$channel" -le 140 ]; then
					op_class=121
				fi
				if [ "$channel" -ge 149 ] && [ "$channel" -le 161 ]; then
					op_class=124
				fi
				if [ "$channel" -gt 161 ] && [ "$channel" -le 169 ]; then
					op_class=125
				fi
			...
			VHT80)
				...
				if [ "$channel" -ge 36 ] && [ "$channel" -le 161 ]; then
					op_class=130
				fi
					case "$channel" in
			...
			VHT160)
				...
				if [ "$channel" -ge 36 ] && [ "$channel" -le 128 ]; then
					op_class=129
				fi
					case "$channel" in
	...
	json_get_vars \
		...
		acs_policy acs_switch_thresh acs_bw_comparison acs_exclude_dfs
...
drv_mac80211_setup_phy() {
	...
	do
		...
		if [ $is_phy_5g = '0' ]; then
			`iw $phy info | grep "Device is meant only for DFS" > /dev/null`
			is_phy_zwdfs=$?

			for i in $path_5g
			do
				`uci show $i | grep "num_antennas" | grep "1" > /dev/null`
				is_radio_zwdfs=$?
