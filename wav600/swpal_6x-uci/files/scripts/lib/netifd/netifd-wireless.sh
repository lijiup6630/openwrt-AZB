...
_wireless_add_vlan() {
	echo "jiale=========_wireless_add_vlan"  > /dev/console
	local name="$1"; shift
	local ifname="$1"; shift
...
for_each_vlan() {
	...
	echo "jiale=========for_each_vlan"  > /dev/console
...
for_each_station() {
	...
	echo "jiale=========for_each_station"  > /dev/console
...
_wdev_common_vlan_config() {
	echo "jiale=========_wdev_common_vlan_config"  > /dev/console
	config_add_string name vid iface
}

_wdev_common_station_config() {
	echo "jiale=========_wdev_common_station_config"  > /dev/console
...
init_wireless_driver() {
	name="$1"; shift
	cmd="$1"; shift
	echo "jiale=========init_wireless_driver ${cmd}"  > /dev/console
	case "$cmd" in
		dump)
			add_driver() {
				eval "drv_$1_cleanup"
