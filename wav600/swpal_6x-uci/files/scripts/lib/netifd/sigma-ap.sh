...
if [ "$OS_NAME" = "RDKB" ]; then
	...
	if [ -n "$atom_cpu" ]; then
		...
		[ "$nof_interfaces" = "4" ] && [ -n `echo "$hw_revision_wlan4" | grep "0x55"` ] && MODEL="AX11000" && cp /lib/netifd/lgm_radio_map /opt/intel/wave/radio_map_file
	else
...
ap_set_wireless()
{
	...
	while [ "$1" != "" ]; do
		...
		SSID)
			...
			if [ "$glob_ssid" = "4.2.3" -a "$OS_NAME" = "RDKB" ] || [ "$glob_ssid" = "4.2.30" -a "$MODEL" = "AX11000" -a "$OS_NAME" = "UGW" ]; then
...
disable_ul_dl_ofdma()
{
	ap_ifaces=`uci show wireless | grep ifname |awk -F"=" '{print $2}' | grep -v "\." | tr -d "'" | sort | uniq -u`
	for ap_iface in $ap_ifaces; do
		debug_print "Disable ul/dl ofdma for $ap_iface"
		ap_tmp=`iw dev $ap_iface iwlwav sDynamicMu 0 0 4 4 0`
	done
}
...
ap_reset_default()
{
	...
	if [ "$MODEL" = "RDKB" ]; then
		if [ $nof_interfaces -ge 3 ]; then
			...
			ap_tmp=`cp -u $DEFAULT_DB_PATH/cgr/radio_map_file_3_cards $DEFAULT_DB_PATH/$ucc_program/`
