	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if (ssid && ssid->id_str)
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CONNECTED
					MACSTR " id=%d id_str=%s%s multi_ap_profile=%d multi_ap_primary_vlanid=%d", MAC2STR(wpa_s->bssid),
					ssid ? ssid->id : -1, ssid->id_str,
					fils_hlp_sent ? " FILS_HLP_SENT" : "", ssid->multi_ap_profile, ssid->multi_ap_primary_vlanid);
		else
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_CONNECTED
					MACSTR " id=%d%s multi_ap_profile=%d multi_ap_primary_vlanid=%d", MAC2STR(wpa_s->bssid), ssid ? ssid->id : -1,
					fils_hlp_sent ? " FILS_HLP_SENT" : "", ssid->multi_ap_profile, ssid->multi_ap_primary_vlanid);
...
skip_ht40:
	...
	freq->he_enabled = ssid->he;
	/*To-do: Need to update chwidth,seg0,seg1 for 6GHz */
	...
	enum wpa_drv_update_connect_params_mask *mask)
{
	...
	wpa_ie = os_zalloc(max_wpa_ie_len);
	if (!wpa_ie) {
		wpa_printf(MSG_ERROR,
...
struct wpa_rtlogger_data * wpa_supplicant_rtlogger_init(struct wpa_rtlogger_config *cfg, const char *ifname)
{
	...
	ret = sprintf_s(cfg->app_name, sizeof(WPA_RTLOGGER_APP_NAME_WPASUPPLICANT),
