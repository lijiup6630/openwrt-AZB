					...
					const u8 *ies, size_t ies_len)
{
	...
	wpa_s->current_ssid->multi_ap_profile1_disallowed = !!(map_sub_elem[2]
					& MULTI_AP_PROFILE1_DISALLOWED);
	wpa_s->current_ssid->multi_ap_profile2_disallowed = !!(map_sub_elem[2]
					& MULTI_AP_PROFILE2_DISALLOWED);

	wpa_s->multi_ap_ie = 1;

	/* Multi-AP Specification v2.0 */
	wpa_s->current_ssid->multi_ap_primary_vlanid = 0;

	/* Multi-AP Profile subelement */
	if (wpa_s->multi_ap_backhaul){
		map_sub_elem = get_ie(pos, len, MULTI_AP_SUB_ELEM_PROFILE);
		if (map_sub_elem && (map_sub_elem[1] == 1) &&
		    ((map_sub_elem[2] == MULTI_AP_PROFILE1) ||
		    (map_sub_elem[2] == MULTI_AP_PROFILE2)))
			wpa_s->current_ssid->multi_ap_profile = map_sub_elem[2];

		/* Multi-AP Default 802.1Q Setting subelement */
		if (wpa_s->current_ssid && wpa_s->current_ssid->multi_ap_backhaul_sta &&
		    wpa_s->conf->multi_ap_profile == MULTI_AP_PROFILE2 &&
		    wpa_s->current_ssid->multi_ap_profile == MULTI_AP_PROFILE2){
			map_sub_elem = get_ie(pos, len, MULTI_AP_SUB_ELEM_DEF_VLAN);
			if ((map_sub_elem && map_sub_elem[1] == 2) &&
			    (le_to_host16((le16)(map_sub_elem[2])) <= VLANID_MAX))
				wpa_s->current_ssid->multi_ap_primary_vlanid = WPA_GET_LE16(map_sub_elem + 2);
		}
	}
}
...
static void multi_ap_set_4addr_mode(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_s->conf->multi_ap_profile == MULTI_AP_PROFILE1 &&
	    wpa_s->current_ssid->multi_ap_profile1_disallowed) ||
	    (wpa_s->conf->multi_ap_profile == MULTI_AP_PROFILE2 &&
	    wpa_s->current_ssid->multi_ap_profile2_disallowed)) {
		wpa_printf(MSG_INFO,
