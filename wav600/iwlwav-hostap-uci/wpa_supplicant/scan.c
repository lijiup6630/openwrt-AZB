			    ...
			    struct wpa_driver_scan_params *params, bool is_6ghz)
{
	...
	for (i = 0; i < mode->num_channels; i++) {
		if (mode->channels[i].flag & HOSTAPD_CHAN_DISABLED)
			continue;
		if (is_6ghz && !is_6ghz_psc_freq(mode->channels[i].freq))
			continue;
	...
	if (is_6ghz) {
		...
		{
			if (is_6ghz_psc_freq(scan_ap_info_tmp->freq))
				continue;
		...
		ieee80211_rnr_info_delete(&wpa_s->global->rnr_scan_6g_ap_info);
	}
				   ...
				   struct wpa_driver_scan_params *params)
{
	...
	if (wpa_s->setband == WPA_SETBAND_5G)
		...
		wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211G, params,
					0);
	if (is_6ghz_freq(wpa_s->hw.modes->channels[0].freq))
		wpa_add_scan_freqs_list(wpa_s, HOSTAPD_MODE_IEEE80211A, params,
					1);
}
...
scan:
	...
	/* This is the point where we trigeer 2.4G/5G scan to fetch RNR report
	 *  when we scan for 6G client mode */
	if (is_6ghz_freq(wpa_s->hw.modes->channels[0].freq))
	{
		struct wpa_supplicant *local = wpa_s->global->ifaces;
		while (local)
		{
			if (local == wpa_s) {
				ret = wpa_supplicant_trigger_scan(wpa_s, scan_params);
			} else {
				wpas_ctrl_scan(local, NULL, (char *)&ret, 0, NULL);
			}
			local=local->next;
		}
	}
			      ...
			      int snr, int freq)
{
	...
	if (hw_mode) {
		...
		struct ieee80211_he_capabilities *own_he;
		u8 cw;
		...
		own_he = &hw_mode->he_capab;

		tmp = max_he_rate(he20_table, snr) + 2;
		if (tmp > est)
			est = tmp;

		cw = he->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] &
			own_he->he_phy_capab_info[HE_PHYCAP_CAP0_IDX];
		if (cw &
		    (IS_2P4GHZ(freq) ? HE_PHY_CAP0_CHANNEL_WIDTH_SET_B0 :
		...
		if (!IS_2P4GHZ(freq) &&
		    (cw & HE_PHY_CAP0_CHANNEL_WIDTH_SET_B1)) {
		...
		if (!IS_2P4GHZ(freq) &&
		    (cw & (HE_PHY_CAP0_CHANNEL_WIDTH_SET_B2 |
...
failed:
	...
	sprintf_s(pos, end - pos, "?");
}


static bool is_rate_exists_in_buff(const u8 *rates_buf, u8 rates_num, u8 rate_ref)
{
	u8 i;
...
static bool is_rate_exists(struct wpa_scan_res *res, u8 rate_ref)
{
	const u8 *ie;
