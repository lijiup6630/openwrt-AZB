				    ...
				    struct os_reltime *fetch_time)
{
	...
	if (!is_6ghz_freq(res->freq)) {
		if(ieee802_11_parse_elems((u8 *)(res + 1), res->ie_len, &elems, 0) == ParseFailed)
			wpa_printf(MSG_DEBUG, "Failed to parse elems from the scan results");
		if (elems.rnr)
			ieee80211_parse_rnr(&elems, &wpa_s->global->rnr_scan_6g_ap_info);
	}
