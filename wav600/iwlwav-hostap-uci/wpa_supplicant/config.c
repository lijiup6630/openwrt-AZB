...
static const struct parse_data ssid_fields[] = {
	...
	{ FUNC_KEY(wep_key2) },
	{ FUNC_KEY(wep_key3) },
	{ INT_RANGE(wep_tx_keyidx, 0, 3) },
#ifdef CONFIG_ALLOW_SUPPLICANT
	{ INT(wep_tx_keyidx) },
#endif /* CONFIG_WEP */
	{ INT(priority) },
#ifdef IEEE8021X_EAPOL
					      ...
					      const char *value)
{
	...
	if(!IS_VALID_U8_RANGE(val)) {
		wpa_printf(MSG_ERROR,
			   "Required connection capability is invalid");
	    return -1;
	}
	proto[cred->num_req_conn_capab] = (u8)val;
			...
			const char *value, int line)
{
	...
	if (os_strcmp(var, "min_dl_bandwidth_home") == 0) {
		cred->min_dl_bandwidth_home = strtoul(value, NULL, 10);
		return 0;
	}

	if (os_strcmp(var, "min_ul_bandwidth_home") == 0) {
		cred->min_ul_bandwidth_home = strtoul(value, NULL, 10);
		return 0;
	}

	if (os_strcmp(var, "min_dl_bandwidth_roaming") == 0) {
		cred->min_dl_bandwidth_roaming = strtoul(value, NULL, 10);
		return 0;
	}

	if (os_strcmp(var, "min_ul_bandwidth_roaming") == 0) {
		cred->min_ul_bandwidth_roaming = strtoul(value, NULL, 10);
		return 0;
	}

	if (os_strcmp(var, "max_bss_load") == 0) {
		tmp = strtoul(value, NULL, 10);
		if(!IS_VALID_RANGE(tmp, 1, 255))
		  return -1;
		cred->max_bss_load = tmp;
		return 0;
	}
	...
	if (os_strcmp(var, "roaming_consortium") == 0) {
		if (!IS_VALID_RANGE(len, 3, sizeof(cred->roaming_consortium))) {
	...
	if (os_strcmp(var, "required_roaming_consortium") == 0) {
		if (!IS_VALID_RANGE(len, 3, sizeof(cred->required_roaming_consortium))) {
			wpa_printf(MSG_ERROR, "Line %d: invalid "
...
#undef ecw2cw
	...
	config->multi_ap_profile = DEFAULT_MULTI_AP_PROFILE;
	return config;
}
...
#define __STR(f) #f, wpa_global_config_parse_str, wpa_config_get_str, OFFSET(f)
#define STR(f) __STR(f), NULL, NULL
#define STR_RANGE(f, min, max) __STR(f), (void *) min, (void *) max
...
static const struct global_parse_data global_fields[] = {
...
#endif /* CONFIG_MESH */
#endif /* CONFIG_ALLOW_SUPPLICANT */
	{ INT_RANGE(disable_scan_offload, 0, 1), 0 },
#ifdef CONFIG_ALLOW_SUPPLICANT
	{ INT(fast_reauth), 0 },
	...
	{ INT_RANGE(wds, 0, 2), 0 },
	{ FUNC(vendor_elems), 0 },
	{ INT_RANGE(multi_ap_profile, 0, 2), 0 },
