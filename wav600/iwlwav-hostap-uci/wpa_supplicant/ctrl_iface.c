...
#else
	if (os_strcasecmp(cmd, "dpp_configurator_params") == 0) {
#endif /* CONFIG_ALLOW_SUPPLICANT */
...
#endif /* CONFIG_WNM */
#endif /* CONFIG_ALLOW_SUPPLICANT */
	#if defined(CONFIG_ALLOW_SUPPLICANT) || defined(CONFIG_DPP)
	} else
	#endif
	{
		value[-1] = '=';
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	const u8 *ie, *ie2, *osen_ie, *p2p, *mesh, *owe, *ve_ie, *rsnxe;
	...
	ret = sprintf_s(pos, end - pos, " freqband=%s", bss->freqband);
	if (ret <= 0) {
		wpa_printf(MSG_ERROR, "%s: sprintf_s error %d", __func__, ret);
		return -1;
	}
	...
	ret = sprintf_s(pos, end - pos, " netmode=%s", bss->netmode);
	if (ret <= 0) {
		wpa_printf(MSG_ERROR, "%s: sprintf_s error %d", __func__, ret);
		return -1;
	}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		printf("%s; *** Received 'GET_RADIO_INFO' (buf= '%s') ***\n",
				__FUNCTION__, buf);
