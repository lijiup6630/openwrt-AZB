			    ...
			    struct ieee80211_he_capabilities *he_cap)
{
	...
	if (is_6ghz_freq(freq)) {
		...
		if (!center_segment0) {
			...
			if (center_segment1) {
				/* For 160MHz,seg1 should be primary center_frequency of 160 and seg0 should be primary center freq of 80MHz
				 * And currently 80+80 MHz configuartion is not supported */
				int bw1 = center_idx_to_bw_6ghz(center_segment1);
				if (bw1 != 3 ||
				    bw != 2) {
					wpa_printf(MSG_ERROR,
						   "6 GHz 160 MHz configuration doesn't use valid channels");
					return -1;
				}
	...
	if (data->vht_enabled || (mode == HOSTAPD_MODE_IEEE80211A && data->he_enabled)) switch (oper_chwidth) {
