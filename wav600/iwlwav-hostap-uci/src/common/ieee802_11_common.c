				      ...
				      int show_errors)
{
	...
	case WLAN_EID_EXT_HE_CAPABILITIES:
		...
		if (elen < (sizeof(capab.he_mac_capab_info)) +
			...
			MIN_HE_MCS_NSS_LEN/*min mcs-nss size*/) {
			wpa_printf(MSG_WARNING, "Ignoring STA HE capabilities due to wrong size: %zu", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_HE_CAP_EXT elen:%zu", elen);
	...
	case WLAN_EID_EXT_HE_OPERATION:
		if (elen > sizeof(struct ieee80211_he_operation)) {
			wpa_printf(MSG_WARNING, "Ignoring STA HE operation IE due to wrong size: %zu", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_HE_OPERATION_EXT elen:%zu", elen);
	...
	case WLAN_EID_EXT_HE_MU_EDCA_PARAMS:
		if (elen > sizeof(struct ieee80211_he_mu_edca_parameter_set)) {
			wpa_printf(MSG_WARNING, "Ignoring STA HE MU EDCA IE due to wrong size: %zu", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_HE_MU_EDCA_EXT elen:%zu", elen);
	...
	case WLAN_EID_EXT_HE_SPATIAL_REUSE_SET:
		if (elen > sizeof(struct ieee80211_he_spatial_reuse_parameter_set)){
			wpa_printf(MSG_WARNING, "Ignoring STA HE spatial reuse IE due to wrong size: %zu", elen);
			break;
		}
		wpa_printf(MSG_EXCESSIVE, "got WLAN_EID_EXT_HE_SPATIAL_REUSE_SET elen:%zu", elen);
	...
	case WLAN_EID_EXT_SHORT_SSID_LIST:
		...
		break;
	case WLAN_EID_EXT_HE_6GHZ_BAND_CAPABILITIES:
...
#define CHANNEL_END_6G    233
	if (op_class_6g) {
		if ((channel >= CHANNEL_START_6G) && (channel <= CHANNEL_END_6G)) {
			res = (STARTING_6G_FREQ_IN_MHZ + (5 * channel));
		}
	} else {
		/* channels 1..14 */
						   ...
						   u8 *op_class, u8 *channel)
{
	...
	/* 6GHz , channel 1 .. 233 */
	if (freq > STARTING_6G_FREQ_IN_MHZ && freq <= LAST_6G_CENTRE_FREQ_BW20_IN_MHZ) {
		int bw;
		u8 idx = (freq - STARTING_6G_FREQ_IN_MHZ) / 5;
...
static int ieee80211_chan_to_freq_global(u8 op_class, u8 chan)
{
	...
	case 135: /* UHB channels, 80+80 MHz: 7, 23, 39.. */
		if (chan < 1 || chan > 233)
			return -1;
		return STARTING_6G_FREQ_IN_MHZ + chan * 5;
...
size_t add_multi_ap_ie(u8 *buf, size_t len, u8 value, u8 profile)
{
	u8 *pos = buf;
...
bool is_6ghz_freq(int freq)
{
	...
	if (center_idx_to_bw_6ghz((freq - STARTING_6G_FREQ_IN_MHZ) / 5) < 0)
		return false;
...
/**
 * is_6ghz_psc_freq - Check if the frequency is a 6 GHz PSC frequency
 * @freq: frequency to check
 *
 * The Preferred Scanning Channels (PSC) are defined in
 * Draft IEEE P802.11ax/D5.0, 26.17.2.3.3
 */
int is_6ghz_psc_freq(int freq)
{
...
size_t multi_ap_add_ie(u8 *buf, size_t len, const u8 *attr, size_t attr_len)
{
	...
	*buf++ = MULTI_AP_OUI_TYPE;
	err = memcpy_s(buf, attr_len, attr, attr_len);
	if (EOK != err) {
...
void ch_width_and_ch_num_to_seg_idx(int bss_width, u8 channel, u8* he_oper_centr_freq_seg0_idx, u8* he_oper_centr_freq_seg1_idx)
{
	switch(bss_width) {
...
void ieee80211_rnr_info_delete(struct dl_list *rnr_scan_6g_ap_info)
{
...
void ieee80211_parse_rnr(struct ieee802_11_elems *elems, struct dl_list *rnr_scan_6g_ap_info)
{
