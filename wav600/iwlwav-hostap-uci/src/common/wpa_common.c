		      ...
		      const u8 *buf, size_t len, u8 *mic)
{
	...
	if (key_len == 0 || key_len > WPA_KCK_MAX_LEN) {
		wpa_printf(MSG_DEBUG,
			   "WPA: KCK not set or wrong len %lu - cannot calculate MIC", (unsigned long) key_len);
		return -1;
	}
...
#define MAX_Z_LEN 66 /* with NIST P-521 */
	...
	if (pmk_len == 0 || pmk_len > PMK_LEN_MAX) {
		wpa_printf(MSG_ERROR, "WPA: No PMK set for PTK derivation or wrong len %lu", (unsigned long) pmk_len);
		return -1;
	}
