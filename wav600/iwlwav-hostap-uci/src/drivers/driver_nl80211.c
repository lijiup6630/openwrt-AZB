	...
	if (ifi->ifi_family == AF_BRIDGE && brid && drv) {
		...
		for (bss = drv->first_bss; bss; bss = bss->next) {
			if (os_strcmp(ifname, bss->ifname) == 0) {
				union wpa_event_data event;
				event.iface_bridge_config.enabled = true;
				os_strlcpy(bss->brname, namebuf, IFNAMSIZ);
				os_strlcpy(event.iface_bridge_config.bridge_name, namebuf, IFNAMSIZ);
				wpa_supplicant_event(bss->ctx, EVENT_LTQ_IFACE_BRIDGE_CONFIG, &event);
				break;
			}
		}
	}
						 ...
						 u8 *buf, size_t len)
{
	...
	if (ifi->ifi_family == AF_BRIDGE && brid && drv) {
		...
		for (bss = drv->first_bss; bss; bss = bss->next) {
			if (os_strcmp(ifname, bss->ifname) == 0) {
				union wpa_event_data event;
				event.iface_bridge_config.enabled = false;
				os_strlcpy(event.iface_bridge_config.bridge_name, namebuf, IFNAMSIZ);
				wpa_supplicant_event(bss->ctx, EVENT_LTQ_IFACE_BRIDGE_CONFIG, &event);
				break;
			}
...
static int nl80211_put_sae_pwe(struct nl_msg *msg, int pwe)
{
	...
	return 0; //This is not supported in V5.10 kernel enable this from V5.11
...
int nl80211_set_mesh_mode(void *priv, const int mesh_mode)
{
	int ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_MESH_MODE,
		(u8*) &mesh_mode, sizeof(mesh_mode), NESTED_ATTR_NOT_USED, NULL);
...
int nl80211_add_backhaul_virtual_iface(void *priv, const char *ifname)
{
	int ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_ADD_BACKHAUL_VRT_IFACE,
				     (u8*) ifname, IFNAMSIZ + 1, NESTED_ATTR_NOT_USED, NULL);

	if (ret < 0)
		wpa_printf(MSG_ERROR, "nl80211: sending ADD_BACKHAUL_VRT_IFACE failed: %i (%s)",
			   ret, strerror(-ret));
...
int nl80211_get_aid(void *priv, u16 *aid, const u8 *addr)
{
	...
	if (res) {
		...
		*aid = 0;
		...
		err = memcpy_s(aid, sizeof(*aid), rsp_aid->buf, aid_size);
		if (EOK != err) {
			wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
			wpabuf_free(rsp_aid);
			return -ENOBUFS;
...
int nl80211_get_he_operation_info(void *priv, struct intel_vendor_he_oper *he_operation)
{
	int res = 0;
	errno_t err;
	size_t he_oper_vendor_size = sizeof(struct intel_vendor_he_oper);
	...
	rsp_he_oper = wpabuf_alloc(he_oper_vendor_size);
	...
	res = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_GET_HE_OPERATION,
				 NULL, 0, NESTED_ATTR_NOT_USED, rsp_he_oper);
	if (res) {
		wpa_printf(MSG_ERROR, "nl80211: sending/receiving HE_OPERATION failed: %i (%s)",
			   res, strerror(-res));
		goto out;
	}

	if (rsp_he_oper->used != he_oper_vendor_size) {
		res = -EMSGSIZE;
		wpa_printf(MSG_ERROR, "nl80211: driver returned %zu bytes instead of expected %zu",
			   rsp_he_oper->used, he_oper_vendor_size);
		goto out;
	}

	err = memcpy_s(he_operation, sizeof(*he_operation), rsp_he_oper->buf, he_oper_vendor_size);
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		res = -ENOBUFS;
		goto out;
	}
	wpa_hexdump(MSG_DEBUG, "Received a new HE_OPERATION", he_operation, sizeof(*he_operation));
...
int nl80211_set_he_operation_info(void *priv, struct intel_vendor_he_oper *he_operation)
{
	int ret;

	wpa_printf(MSG_DEBUG, " nl80211: sending SET_HE_OPERATION ");

	ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_HE_OPERATION,
				(u8 *)he_operation, sizeof(*he_operation), NESTED_ATTR_NOT_USED, NULL);
...
int nl80211_get_he_non_advertised_info(void *priv, struct intel_vendor_he_capa *non_advertised_he_caps)
{
	int res = 0;
	...
	rsp_he_non_advert = wpabuf_alloc(sizeof(*non_advertised_he_caps));
	...
	res = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_GET_HE_NON_ADVERTISED,
					NULL, 0, NESTED_ATTR_NOT_USED, rsp_he_non_advert);
	if (res) {
		wpa_printf(MSG_ERROR, "nl80211: sending/receiving HE non-advertised capabilities failed: %i "
		"(%s)", res, strerror(-res));
		goto out;
	}
	if (rsp_he_non_advert->used != sizeof(*non_advertised_he_caps)) {
		res = -EMSGSIZE;
		wpa_printf(MSG_ERROR, "nl80211: driver returned %zu bytes instead of %d",
		rsp_he_non_advert->used, (int)sizeof(*non_advertised_he_caps));
		goto out;
	}

	if (EOK != memcpy_s(non_advertised_he_caps, sizeof(*non_advertised_he_caps),
			    rsp_he_non_advert->buf, rsp_he_non_advert->used)) {
		res = -ENOBUFS;
		goto out;
	}
	wpa_hexdump(MSG_DEBUG, "Received a new HE non-advertised capabilities",
		    non_advertised_he_caps, sizeof(*non_advertised_he_caps));
...
int nl80211_set_he_non_advertised_info(void *priv, struct intel_vendor_he_capa *he_capab)
{
	int ret;

	ret = nl80211_vendor_cmd(priv, OUI_LTQ,
				 LTQ_NL80211_VENDOR_SUBCMD_SET_HE_NON_ADVERTISED,
				 (u8 *)he_capab, sizeof(*he_capab), NESTED_ATTR_NOT_USED, NULL);
	if (ret < 0)
		wpa_printf(MSG_ERROR,
  ...
  struct intel_vendor_sta_info *sta_info)
{
  ...
  ret = nl80211_vendor_cmd(priv, OUI_LTQ,
    LTQ_NL80211_VENDOR_SUBCMD_GET_STA_MEASUREMENTS, sta_addr, ETH_ALEN, NESTED_ATTR_NOT_USED, rsp);
  ...
  err = memcpy_s(sta_info, sizeof(*sta_info), rsp->buf, sizeof(*sta_info));
  if (EOK != err) {
    wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
    return -ENOBUFS;
  }
  wpa_printf(MSG_EXCESSIVE, "nl80211: Received station measurements for station "
		...
		struct intel_vendor_vap_info *vap_info)
{
  ...
  ret = nl80211_vendor_cmd(priv, OUI_LTQ,
    LTQ_NL80211_VENDOR_SUBCMD_GET_VAP_MEASUREMENTS, NULL, 0, NESTED_ATTR_NOT_USED, rsp);
  ...
  err = memcpy_s(vap_info, sizeof(*vap_info), rsp->buf, sizeof(*vap_info));
  if (EOK != err) {
    wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
    ret = -ENOBUFS;
	goto out;
  }
  wpa_printf(MSG_EXCESSIVE, "nl80211: Received VAP measurements");
...
int nl80211_get_radio_info(void *priv, struct intel_vendor_radio_info *radio_info)
{
  ...
  ret = nl80211_vendor_cmd(priv, OUI_LTQ,
    LTQ_NL80211_VENDOR_SUBCMD_GET_RADIO_INFO, NULL, 0, NESTED_ATTR_NOT_USED, rsp);
  ...
  err = memcpy_s(radio_info, sizeof(*radio_info), rsp->buf, sizeof(*radio_info));
  if (EOK != err) {
    wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
    ret = -ENOBUFS;
	goto out;
  }
  wpa_printf(MSG_EXCESSIVE, "nl80211: Received radio info");
					...
					struct intel_vendor_he_capa *data)
{
	...
	ret = nl80211_vendor_cmd(priv, OUI_LTQ,
				 ...
				 (u8 *)data,
				 sizeof(struct intel_vendor_he_capa), NESTED_ATTR_NOT_USED, NULL);
	if (ret < 0)
		wpa_printf(MSG_ERROR,
...
int nl80211_set_unsolicited_frame(void *priv, int mode, int duration)
{
	int ret;
	int unsolicited_frame_cfg[] = {mode, duration};

	ret = nl80211_vendor_cmd(priv, OUI_LTQ,
				 LTQ_NL80211_VENDOR_SUBCMD_SET_UNSOLICITED_FRAME_TX,
				 (u8 *)unsolicited_frame_cfg, sizeof(unsolicited_frame_cfg),
				  NESTED_ATTR_NOT_USED, NULL);
	if (ret < 0)
		wpa_printf(MSG_ERROR,
			   "nl80211: SET_UNSOLICITED_FRAME CMD failed: %i (%s)",
			   ret, strerror(-ret));
	return ret;
}
...
int nl80211_send_atf_quotas(void *priv, struct intel_vendor_atf_quotas* atf_quotas)
{
  int ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_ATF_QUOTAS,
		  (u8*)atf_quotas, sizeof(struct intel_vendor_atf_quotas) + atf_quotas->data_len,
		  NESTED_ATTR_NOT_USED, NULL);
			      ...
			      struct wpabuf *buf)
{
	...
	if (OUI_LTQ == vendor_id)
		ret = send_and_recv_msgs(drv, msg, vendor_ltq_reply_handler, buf,
					 NULL, NULL);
	else
...
int nla_put_u8_hapd(struct nlmsghdr *n, int type, __u8 data)
{
	return nla_put_hapd(n, MAX_DATA_LEN, type, &data, sizeof(__u8));
}

int nla_nest_end_hapd(struct nlmsghdr *n, struct rtattr *nest)
{
	nest->rta_len = (char *)NLA_TAIL(n) - (char *)nest;
	return n->nlmsg_len;
}

struct rtattr *nla_nest_start_hapd(struct nlmsghdr *n, __u32 maxlen, int type)
{
...
int nl80211_get_zwdfs_antenna(void *priv, int *enabled)
{
	int ret;
	errno_t err;
	struct wpabuf *rsp;

	rsp = wpabuf_alloc(sizeof(*enabled));
	if (!rsp)
		return -ENOBUFS;

	ret = nl80211_vendor_cmd(priv, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_GET_ZWDFS_ANT,
				     NULL, 0, NESTED_ATTR_NOT_USED, rsp);
	if (ret < 0) {
		wpa_printf(MSG_ERROR, "nl80211: sending GET_ZWDFS_ANTENNA failed: %i (%s)",
			   ret, strerror(-ret));
		goto out;
	}

	if (rsp->used != sizeof(*enabled)) {
		wpa_printf(MSG_ERROR, "nl80211: driver returned %zu bytes instead of %zu",
			   rsp->used, sizeof(*enabled));
		ret = -EMSGSIZE;
		goto out;
	}

	err = memcpy_s(enabled, sizeof(*enabled), rsp->buf, sizeof(*enabled));
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		ret = -ENOBUFS;
		goto out;
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.add_backhaul_vrt_iface = nl80211_add_backhaul_virtual_iface,
	...
	.get_zwdfs_antenna = nl80211_get_zwdfs_antenna,
	.set_chan_dfs_state = nl80211_set_channel_dfs_state,
	.set_unsolicited_frame = nl80211_set_unsolicited_frame,
