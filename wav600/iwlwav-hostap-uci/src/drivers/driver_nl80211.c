				      ...
				      struct wpa_driver_set_key_params *params)
{
	...
	if (!msg ||
		nla_put_u8(msg, NL80211_ATTR_KEY_IDX, key_idx) ||
	    	nla_put_flag(msg, (alg == WPA_ALG_BIP_CMAC_128 ||
...
int nl80211_get_aid(void *priv, u16 *aid, const u8 *addr)
{
	...
	rsp_aid = wpabuf_alloc(8); /* aid_size + NLA_HDRLEN + NLA_ALIGN */
	if (!rsp_aid) {
		return -ENOBUFS;
	}
...
int nl80211_get_he_operation_info(void *priv, struct intel_vendor_he_oper *he_operation)
{
	...
	rsp_he_oper = wpabuf_alloc(NLA_ALIGN(sizeof(struct nlattr) + he_oper_vendor_size));
...
int nl80211_set_he_non_advertised_info(void *priv, struct intel_vendor_he_capa *he_capab)
{
	...
	ret = nl80211_vendor_cmd(priv, OUI_LTQ,
				 ...
				 (u8 *)he_capab, sizeof(*he_capab), NULL);
	if (ret < 0)
		wpa_printf(MSG_ERROR,
  ...
  struct intel_vendor_sta_info *sta_info)
{
  ...
  /* sizeof(*peer_stats) + NLA_HDRLEN + NLA_ALIGN */
		...
		struct intel_vendor_vap_info *vap_info)
{
  ...
  /* sizeof(*vap_info) + NLA_HDRLEN + NLA_ALIGN */
...
int nl80211_get_radio_info(void *priv, struct intel_vendor_radio_info *radio_info)
{
  ...
  /* sizeof(*radio_info) + NLA_HDRLEN + NLA_ALIGN */
					...
					struct intel_vendor_he_capa *data)
{
	...
	ret = nl80211_vendor_cmd(priv, OUI_LTQ,
				 ...
				 (u8 *)data,
				 sizeof(struct intel_vendor_he_capa), NULL);
	if (ret < 0)
		wpa_printf(MSG_ERROR,
...
int nl80211_get_zwdfs_antenna(void *priv, int *enabled)
{
	...
	/* sizeof(*enabled) + NLA_HDRLEN + NLA_ALIGN */
	rsp = wpabuf_alloc(NLA_ALIGN(sizeof(struct nlattr) + sizeof(*enabled)));
	if (!rsp)
		return -ENOBUFS;
