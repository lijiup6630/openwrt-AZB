...
struct wpa_driver_ops {
  ...
  /**
   * add_backhaul_vrt_iface - backhaul virtual interface creation request to driver
   * @priv: Private driver interface data
   * @ifname: name of the interface to create
   * Returns: 0 on success, -1 on failure
   */
  int (*add_backhaul_vrt_iface)(void *priv, const char *ifname);
  ...
  int (*set_antenna)(void *priv, u32 tx_ant, u32 rx_ant);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int(*get_he_operation_info)(void *priv, struct intel_vendor_he_oper *he_operation);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int(*set_he_operation_info)(void *priv, struct intel_vendor_he_oper *he_operation);
	 ...
	 * Returns: 0 on success, -1 on failure
	 */
	int (*send_ltq_he_debug_mode_data)(void *priv, struct intel_vendor_he_capa *data);
	...
	* Returns: 0 on success, -1 on failure
	*/
        int(*get_he_non_advertised_info)(void *priv, struct intel_vendor_he_capa *non_advertised_he_caps);
	...
	*/
	...
	/**
	* get_zwdfs_antenna - get ZWDFS antenna Enabled/Disabled if supported.
	* @priv: Private driver interface data
	* @enable: Flag indicating enable/disable ZWDFS antenna
	* Returns: 0 on success, -1 on failure
	*/
	int (*get_zwdfs_antenna)(void *priv, int *enabled);

	/**
	* get_tx_power_20mhz - get tx power for 20mhz band width
	* @priv: Private driver interface data
	* Returns: 0 on success, -1 on failure
	*/
	...
	/**
	 * set_chan_dfs_state - sets a channel dfs state
	 * @priv: Private driver interface data
	 * @dfs_state: dfs state
	 * @freq : freq of the channel
	 * @dfs_time : time in dfs unavailable state
	 * @dfs_debug : debug mode, without any restrictions
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_chan_dfs_state)(void *priv,enum nl80211_dfs_state dfs_state,
				  int freq, int dfs_time, int dfs_debug);
	/**
	 * set_unsolicited_frame - start transmitting unsolicited frame
	 * @priv: Private driver interface data
	 * @mode: 0: Disable 1: 20TU Probe Response 2: FILS Discovery
	 * @duration: Interval between successive frame transmissions
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_unsolicited_frame)(void *priv, int mode, int duration);
...
enum wpa_event_type {
	...
	/**
	 * EVENT_DFS_CAC_PAUSED - Notify that channel availability check has been paused
	 *
	 * The CAC paused, probably because a scan request was initiated by the user.
	 * Hostapd should restart CAC when interface is no longer busy.
	 */
	EVENT_DFS_CAC_PAUSED,
	/**
...
union wpa_event_data {
	...
	struct intel_vendor_event_coc coc;
...
static inline void drv_event_ltq_softblock_drop(void *ctx, const u8 *data, size_t data_len)
{
	...
	if (!data || (data_len != sizeof(event.msg_dropped))) {
		wpa_printf(MSG_ERROR, "Received LTQ vendor Softblock drop wrong data size");
		return;
	}

	err = memcpy_s(&event.msg_dropped, sizeof(event.msg_dropped), data, data_len);
	if (EOK != err) {
...
static inline void drv_event_ltq_coc(void *ctx, const u8 *data, size_t data_len)
{
	union wpa_event_data event;
	os_memset(&event, 0, sizeof(event));
	os_memcpy(&event.coc, data, data_len);
	wpa_supplicant_event(ctx, EVENT_LTQ_COC, &event);
}
...
static inline void drv_event_ltq_radar (void *ctx, const u8 *data, size_t data_len)
{
	...
	union wpa_event_data event;
	os_memset(&event, 0, sizeof(event));
	err = memcpy_s(&event.radar, sizeof(event.radar), data, data_len);
	if (EOK != err) {
		wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
		return;
	}
