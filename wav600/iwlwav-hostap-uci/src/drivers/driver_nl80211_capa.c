...
static int phy_info_he_caps(struct hostapd_hw_modes *mode, struct nlattr *tb)
{
	...
	nla_for_each_nested(nl_he_cap, tb, rem_iftype) {
		...
		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MAC]) {
			err = memcpy_s(&mode->he_capab.he_mac_capab_info, sizeof(mode->he_capab.he_mac_capab_info),
		...
		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PHY]) {
			err = memcpy_s(&mode->he_capab.he_phy_capab_info, sizeof(mode->he_capab.he_phy_capab_info),
		...
		mcs_size = MIN(MAX_HE_MCS_LEN, nla_len(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MCS_SET]));

		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_MCS_SET]) {
			err = memcpy_s(&mode->he_capab.optional,mcs_size,
		...
		ppe_thresh_size = MIN(HE_PPE_THRES_MAX_LEN, nla_len(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PPE]));

		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_CAP_PPE]) {
			err = memcpy_s((u8 *)(&mode->he_capab.optional[mcs_size]),ppe_thresh_size,
		...
		if (tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_6GHZ_CAPA]) {
			mode->he_6ghz_capab.capab = nla_get_u16(tb_iftype[NL80211_BAND_IFTYPE_ATTR_HE_6GHZ_CAPA]);
		}
...
static int phy_info_band(struct phy_info_arg *phy_info, struct nlattr *nl_band)
{
	...
	if (phy_info->last_mode != nl_band->nla_type) {
		...
		os_memset(&mode->he_capab.optional, 0xff, MAX_HE_MCS_LEN);
	...
	ret = phy_info_he_caps(mode, tb_band[NL80211_BAND_ATTR_IFTYPE_DATA]);
	if (ret != NL_OK) {
		phy_info->failed = 1;
		return ret;
	}
