 ...
 *  \brief Driver interaction with Linux nl80211/cfg80211 - Event processing
 */
				...
				struct nlattr **tb)
{
	...
	case NL80211_RADAR_CAC_PAUSED:
		wpa_supplicant_event(drv->ctx, EVENT_DFS_CAC_PAUSED, &data);
		break;
...
/******************************************************************************/
/*! \brief      Handle Softblock drop vendor event from driver
 *
 *  \param[in]  drv      pointer to wpa_driver_nl80211_data
 *  \param[in]  data     pointer to data, must not be NULL
 *  \param[in]  len      data size, must be sizeof(struct intel_vendor_event_msg_drop)
 *
 *  \note       \a drv is not NULL
 *
 *  \return     void
...
/******************************************************************************/
/*! \brief      Handle Channel data vendor event from driver
 *
 *  \param[in]  drv      pointer to wpa_driver_nl80211_data
 *  \param[in]  data     pointer to data, must not be NULL
 *  \param[in]  len      data size, must be sizeof(struct intel_vendor_channel_data)
 *
 *  \note       \a drv is not NULL
 *
 *  \return     void
...
static void ltq_nl80211_handle_chan_data(struct wpa_driver_nl80211_data *drv,
				const u8 *data, size_t len)
{
	wpa_printf(MSG_DEBUG, "nl80211: Receive LTQ vendor event: channel data, ctx=%p", drv->ctx);

	drv_event_ltq_chan_data(drv->ctx, data, len);
}

static void ltq_nl80211_handle_coc(struct wpa_driver_nl80211_data *drv,
    const u8 *data, size_t len)
{
  wpa_printf(MSG_INFO, "nl80211: Receive LTQ vendor event:COC, ctx=%p",
      drv->ctx);

  drv_event_ltq_coc(drv->ctx, data, len);
}
...
/******************************************************************************/
/*! \brief      Handle Legacy WDS connect vendor event from driver
 *
 *  \param[in]  drv      pointer to wpa_driver_nl80211_data
 *  \param[in]  data     pointer to data, must not be NULL
 *  \param[in]  len      data size, must be at least sizeof(struct intel_vendor_wds_sta_info)
 *
 *  \note       \a drv is not NULL
 *
 *  \return     void
...
/******************************************************************************/
/*! \brief      Handle Legacy WDS disconnect vendor event from driver
 *
 *  \param[in]  drv      pointer to wpa_driver_nl80211_data
 *  \param[in]  data     pointer to data, must not be NULL
 *  \param[in]  len      data size, must be at least sizeof(struct intel_vendor_wds_sta_info)
...
/******************************************************************************/
/*! \brief      Handle Radar vendor event from driver
 *
 *  \param[in]  drv      pointer to wpa_driver_nl80211_data
 *  \param[in]  data     pointer to data, must not be NULL
 *  \param[in]  len      data size, must be sizeof(struct intel_vendor_radar)
 *
 *  \note       \a drv is not NULL
 *
 *  \return     void
				     ...
				     u32 subcmd, u8 *data, size_t len)
{
	switch (subcmd) {
		...
		case LTQ_NL80211_VENDOR_EVENT_COC_BEACON_UPDATE:
			ltq_nl80211_handle_coc(drv, data, len);
			break;
		default:
		...
		for (bss = drv->first_bss; bss; bss = bss->next) {
			...
			if ((ifidx == -1 && !wiphy_idx_set && !wdev_id_set) ||
			    ...
			    (wdev_id_set && bss->wdev_id_set &&
			     wdev_id == bss->wdev_id) ||
			    (gnlh->cmd == NL80211_CMD_REG_CHANGE)) {
