			...
			int rssi, int from_queue, int snr_db)
{
	...
	u16 seq_ctrl;
	int softblock_res = HOSTAPD_SOFTBLOCK_ACCEPT, sb_acl;
	...
	/* Soft Block ACL */
	sb_acl = hostapd_soft_block_acl_client_status(hapd, mgmt->sa, WLAN_FC_STYPE_AUTH, &resp);
	...
	if (hapd->conf->authresp_elements) {
		...
		err = memcpy_s(pos, vendor_ie_len, wpabuf_head(hapd->conf->authresp_elements), vendor_ie_len);
		if (EOK != err) {
...
#ifdef CONFIG_SAE
	/* sending SAE commit with status code as UNSPECIFIED_FAILURE (0x1) */
	if ((auth_alg == WLAN_AUTH_SAE) && (resp == WLAN_STATUS_UNSPECIFIED_FAILURE)) {
		reply_res = send_auth_reply(hapd, sta, mgmt->sa, mgmt->bssid, auth_alg,
								auth_transaction, resp, resp_ies,resp_ies_len, "handle-auth");
		goto ie_free;
	}
#endif /* CONFIG_SAE */
	reply_res = send_auth_reply(hapd, sta, mgmt->sa, mgmt->bssid, auth_alg,
			  ...
			  const u8 *multi_ap_ie, size_t multi_ap_len)
{
	...
	if (multi_ap_ie) {
		...
		if (multi_ap_subelem && multi_ap_subelem[1] == 1) {
			...
			hostapd_logger(hapd, sta->addr,
				       HOSTAPD_MODULE_IEEE80211,
				       HOSTAPD_LEVEL_INFO,
				       "Multi-AP IE has missing or invalid Multi-AP Extension subelement");
			return WLAN_STATUS_INVALID_IE;
		}

		multi_ap_subelem = get_ie(multi_ap_ie + 4,
					  multi_ap_len - 4,
					  MULTI_AP_SUB_ELEM_PROFILE);
		if (multi_ap_subelem){
			if (multi_ap_subelem[1] == 1) {
				multi_ap_profile = multi_ap_subelem[2];

			} else {
				hostapd_logger(hapd, sta->addr,
					       HOSTAPD_MODULE_IEEE80211,
					       HOSTAPD_LEVEL_INFO,
					       "Multi-AP IE has invalid Multi-AP Profile subelement");
				return WLAN_STATUS_INVALID_IE;
			}
		}
	}
	...
	if (((!multi_ap_profile || (multi_ap_profile == MULTI_AP_PROFILE1)) &&
	    hapd->conf->multi_ap_profile1_disallow) ||
	    (multi_ap_profile == MULTI_AP_PROFILE2 && hapd->conf->multi_ap_profile2_disallow)){
		hostapd_logger(hapd, sta->addr,
			       HOSTAPD_MODULE_IEEE80211,
			       HOSTAPD_LEVEL_INFO,
			       "Multi-AP STA profile disallowed");
		return WLAN_STATUS_ASSOC_DENIED_UNSPEC;
	}

	sta->multi_ap_profile = multi_ap_profile;
	sta->flags |= WLAN_STA_MULTI_AP;
	return WLAN_STATUS_SUCCESS;
}
#ifdef CONFIG_WDS_WPA
...
u16 copy_bss_max_idle_period_from_sta(struct hostapd_data *hapd, struct sta_info *sta,
			   ...
			   const u8 *ies, size_t ies_len, int rssi)
{
	...
	if (sta && hapd->iconf->assoc_rsp_rx_mcs_mask) {
		...
		if (!p) {
			wpa_printf(MSG_ERROR, "hostapd_eid_ht_capabilities_assoc_resp failed");
			res = WLAN_STATUS_UNSPECIFIED_FAILURE;
			goto done;
		}
	} else {
	...
	if ((hapd->iconf->ieee80211ac && !hapd->conf->disable_11ac && !is_6ghz_op_class(hapd->iconf->op_class)) ||
		...
		if (!hapd->iconf->ieee80211n_acax_compat || (sta && (sta->flags & WLAN_STA_VHT))) {
			u32 nsts = 0, sta_nsts;
...
rsnxe_done:
	...
	if (hapd->conf->mesh_mode == MESH_MODE_EXT_HYBRID ||
		...
		if (elems80211.brcom_ie &&
				...
				((size_t) (buf + buflen - p) >= elems80211.brcom_ie_len)) {
			*p++ = WLAN_EID_VENDOR_SPECIFIC;
			*p++ = elems80211.brcom_ie_len;
			os_memcpy(p, elems80211.brcom_ie, elems80211.brcom_ie_len);
			p += elems80211.brcom_ie_len;
		}
		if (elems80211.intel_ie_4addr_mode &&
				...
				((size_t) (buf + buflen - p) >= elems80211.intel_ie_4addr_len)) {
			*p++ = WLAN_EID_VENDOR_SPECIFIC;
			*p++ = elems80211.intel_ie_4addr_len;
			os_memcpy(p, elems80211.intel_ie_4addr_mode, elems80211.intel_ie_4addr_len);
			p += elems80211.intel_ie_4addr_len;
		}
	}
			 ...
			 int reassoc, int rssi)
{
	...
	if ((hapd->conf->mesh_mode == MESH_MODE_BACKHAUL_AP && !sta->four_addr_mode_sta) ||
	    (hapd->conf->mesh_mode == MESH_MODE_FRONTHAUL_AP && sta->four_addr_mode_sta &&
	     !(hapd->conf->mesh_mode_set && sta->multi_ap_supported && (sta->flags & (WLAN_STA_WPS | WLAN_STA_MAYBE_WPS)))))
	{
		...
		wpa_printf(MSG_DEBUG,
			"AP mesh_mode is %s , STA interface is %s, sta_flags = %d ",
		  	(hapd->conf->mesh_mode == MESH_MODE_FRONTHAUL_AP)? "fAP" : "bAP",
			(sta->four_addr_mode_sta)? "4addr" : "3addr", sta->flags);
		resp = WLAN_STATUS_DENIED_INSUFFICIENT_BANDWIDTH;
		goto fail;
	}
			 ...
			 unsigned int freq, int ssi_signal)
{
	...
	case WLAN_ACTION_PROTECTED_DUAL:
		...
		if (len >= IEEE80211_HDRLEN + 2 &&
			...
			hostapd_handle_action_extcsa(hapd, (const u8 *) mgmt, len,
					ssi_signal);
			return 1;
		}
	...
	case WLAN_ACTION_SPECTRUM_MGMT:
		hostapd_handle_spectrum_management(hapd, (const u8 *) mgmt, len,
				ssi_signal);
		return 1;
	}
			      ...
			      size_t len)
{
	...
	if (hex) {
		...
		wpa_msg_ctrl(hapd->msg_ctx, MSG_INFO,
			     AP_MGMT_FRAME_RECEIVED_TRIG "buf=%s", hex);
		os_free(hex);
	}
}
		    ...
		    struct hostapd_frame_info *fi)
{
	...
	if (fi && (stype == WLAN_FC_STYPE_BEACON)) {
...
u8 * hostapd_eid_txpower_envelope(struct hostapd_data *hapd, u8 *eid)
{
	...
	if (is_6ghz_op_class(iface->conf->op_class))
		return hostapd_eid_txpower_envelope_6g(hapd, eid, only_one_psd_limit_in_6g);
