  ...
  *pos++ = sizeof(*cap);
  ...
  if (sta->ht_capabilities) {
    for (i = 0; i < sizeof(cap->supported_mcs_set); i++) {
      ...
      err = memcpy_s(cap->supported_mcs_set, sizeof(cap->supported_mcs_set),
	       hapd->iface->current_mode->mcs_set, 16);
      if (EOK != err) {
...
u8 * hostapd_eid_ht_capabilities(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = sizeof(*cap);
	...
	if(hapd->iconf->ht_mcs_set[0]) {
		err = memcpy_s(cap->supported_mcs_set, sizeof(cap->supported_mcs_set),
			 &hapd->iconf->ht_mcs_set[0], sizeof(cap->supported_mcs_set));
		if (EOK != err) {
			wpa_printf(MSG_ERROR, "%s: memcpy_s error %d", __func__, err);
			return NULL;
		}
...
u8 * hostapd_eid_ht_operation(struct hostapd_data *hapd, u8 *eid)
{
	...
	*pos++ = sizeof(*oper);
	...
	if (hapd->iconf->tx_mcs_set_defined)
		oper->basic_mcs_set[12] |= HT_INFO_HT_PARAM_TX_MCS_SET_DEFINED;
	pos += sizeof(*oper);
	...
	if (iface->drv_flags & WPA_DRIVER_FLAGS_HT_2040_COEX) {
		if (iface->conf->secondary_channel) {
			...
			wpa_msg(hapd->msg_ctx, MSG_INFO, AP_BW_CHANGED "freq=%d Channel=%d "
				"OperatingChannelBandwidth=%s ExtensionChannel=%d cf1=%d cf2=%d reason=%s dfs_chan=%d",
				hapd->iface->freq, hapd->iconf->channel, "20", 0, hapd->iface->freq, 0, "OBSS",	0);
		}
...
void hostapd_ltq_update_coc(struct hostapd_iface *iface, struct intel_vendor_event_coc *bw_params, size_t data_len)
{
	unsigned int i;
	u8 bw = 0;
	u8 center_channels_160[] = { 50, 114 };
	u8 center_channels_80[] = { 42, 58, 106, 122, 138, 155 };

	if (bw_params->coc_BW == BW_MOVE_UP && iface->fallback_coc_freq != 0) {
		bw = iface->fallback_coc_freq;
		iface->conf->secondary_channel = iface->fb_coc_sec_chan;
		iface->fallback_coc_freq = 0; /* no need to perform any more ch switches */
	}

	if (bw_params->coc_BW == BW_MOVE_DOWN) {
		bw = BW_20;
		iface->fb_coc_sec_chan = iface->conf->secondary_channel;
		iface->conf->secondary_channel = 0;
		switch (hostapd_get_oper_chwidth(iface->conf)) {
			case CHANWIDTH_USE_HT:
				if (iface->fb_coc_sec_chan == 0) {
					iface->fallback_coc_freq = BW_20;
				} else {
					iface->fallback_coc_freq = BW_40;
				}
				break;
			case CHANWIDTH_80MHZ:
				iface->fallback_coc_freq = BW_80;
				break;
			case CHANWIDTH_160MHZ:
				iface->fallback_coc_freq = BW_160;
				break;
		}
	}

	/* Irrespective of BW switch up or down send the CSA IE
	 * but if no BW switch request comes to hostapd from driver
	 * then dont send the CSA also */
	if (bw)
		iface->require_csa = 1;

	wpa_printf(MSG_INFO,"BW = %d, iface->fallback_coc_freq = %d, spatial streams = %d fb_sec_chan = %d\n"
			, bw_params->coc_BW, iface->fallback_coc_freq, bw_params->coc_omn_IE, iface->fb_coc_sec_chan);

	switch (bw) {
		case BW_20:
			wpa_printf(MSG_INFO,"COC BW setting 20Mhz \n");
			hostapd_set_oper_chwidth(iface->conf, CHANWIDTH_USE_HT);
			hostapd_set_oper_centr_freq_seg0_idx(iface->conf, 0);
			hostapd_set_oper_centr_freq_seg1_idx(iface->conf, 0);
			break;
		case BW_40:
			wpa_printf(MSG_INFO,"COC BW setting 40Mhz \n");
			hostapd_set_oper_chwidth(iface->conf, CHANWIDTH_USE_HT);
			hostapd_set_oper_centr_freq_seg0_idx(iface->conf, 0);
			hostapd_set_oper_centr_freq_seg1_idx(iface->conf, 0);
			break;
		case BW_80:
			wpa_printf(MSG_INFO,"COC BW setting 80Mhz \n");
			hostapd_set_oper_chwidth(iface->conf, CHANWIDTH_80MHZ);
			hostapd_set_oper_centr_freq_seg1_idx(iface->conf, 0);
			for (i = 0; i < ARRAY_SIZE(center_channels_80); i++){
				if (iface->conf->channel >= center_channels_80[i] - 6 &&
						iface->conf->channel <= center_channels_80[i] + 6){
					hostapd_set_oper_centr_freq_seg0_idx(iface->conf, center_channels_80[i]);
				}
			}
			break;
		case BW_160:
			wpa_printf(MSG_INFO,"COC BW setting 160Mhz \n");
			hostapd_set_oper_chwidth(iface->conf, CHANWIDTH_160MHZ);
			hostapd_set_oper_centr_freq_seg1_idx(iface->conf, 0);
			for (i = 0; i < ARRAY_SIZE(center_channels_160); i++){
				if (iface->conf->channel >= center_channels_160[i] - 14 &&
						iface->conf->channel <= center_channels_160[i] + 14){
					hostapd_set_oper_centr_freq_seg0_idx(iface->conf, center_channels_160[i]);
				}
			}
			break;
		default:
			if (iface->require_omn == bw_params->coc_omn_IE) /* nothing to do */
				return;

			wpa_printf(MSG_INFO,"Update OMN IE in Beacons with spatial streams = %d  \n", iface->require_omn);
			break;
	}

	iface->require_omn = bw_params->coc_omn_IE;
	eloop_cancel_timeout(ap_max_nss_omn_elem_timeout, iface, NULL);

	ieee802_11_update_beacons(iface);

	if (bw_params->coc_is_max_nss)
		eloop_register_timeout(2, 0, ap_max_nss_omn_elem_timeout, iface, NULL);
}
			 ...
			 int rssi_level)
{
	...
	} else if (!elems->ht_capabilities && !elems->vht_capabilities && !elems->pc_he_capabilities){
...
void ap_ht2040_timeout(void *eloop_data, void *user_data)
{
	...
	ieee802_11_update_beacons(iface);
	wpa_msg(hapd->msg_ctx, MSG_INFO, AP_BW_CHANGED "freq=%d Channel=%d OperatingChannelBandwidth=%s ExtensionChannel=%d cf1=%d "
			"cf2=%d reason=%s dfs_chan=%d", hapd->iface->freq, hapd->iconf->channel, "40", iface->conf->secondary_channel, hapd->iface->freq, 0,
			"OBSS", 0);
}
