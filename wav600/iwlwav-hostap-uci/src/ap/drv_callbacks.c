			     ...
			     int finished)
{
	...
	/* chwidth defination of 6G is different from 2.4Ghz and 5Ghz.
	 * So we will use the 6G defination of chwidth.
	 * */
	op_class_6g = is_6ghz_op_class(hapd->iface->conf->op_class);
	switch (width) {
		case CHAN_WIDTH_80:
			chwidth = op_class_6g ? HE_OPER_CHANWIDTH_80 : CHANWIDTH_80MHZ;
			break;
		case CHAN_WIDTH_80P80:
			chwidth = op_class_6g ? HE_OPER_CHANWIDTH_80 : CHANWIDTH_80P80MHZ;
			break;
		case CHAN_WIDTH_160:
			chwidth = op_class_6g ? HE_OPER_CHANWIDTH_160 : CHANWIDTH_160MHZ;
			break;
		case CHAN_WIDTH_40:
			chwidth = op_class_6g ? HE_OPER_CHANWIDTH_40 : CHANWIDTH_USE_HT;
			break;
		case CHAN_WIDTH_20_NOHT:
		case CHAN_WIDTH_20:
		default:
			chwidth = op_class_6g ? HE_OPER_CHANWIDTH_20 : CHANWIDTH_USE_HT;
			break;
	}
	...
	/* We should update the RNR info of lower bands on co-located BSS,
	 * whenever there is switch on 6G channel.
	 * Along with RNR we need to update HE oper 6G information field in Beacons,
	 * to reflect the correct seg0 and seg1 idx of new channel.
	 * */
	if (op_class_6g) {
		hapd->iface->conf->he_oper.vht_op_info_chwidth = chwidth;
		hapd->iface->conf->he_oper.vht_op_info_chan_center_freq_seg0_idx = hostapd_get_oper_centr_freq_seg0_idx(hapd->iface->conf);
		hapd->iface->conf->he_oper.vht_op_info_chan_center_freq_seg1_idx = hostapd_get_oper_centr_freq_seg1_idx(hapd->iface->conf);
		if ( hapd->conf->rnr_auto_update && auto_update_rnr_in_beacon(hapd->iface)) {
				  ...
				  const char *msg_str)
{
	...
	while (e != NULL){
		if ((os_memcmp(e->addr, sta_mac, ETH_ALEN) == 0) &&
		    is_same_softblock_event(msg_str, e->msg_string))
			break;
		e = e->hnext;
	}
	return e;
