...
extern channel_80   g_channels80_6G[NUM_CHAN_PAIRS_80_6G];
extern channel_160  g_channels160_6G[NUM_CHAN_PAIRS_160_6G];
...
int get_he_width(int width)
{
  switch (width) {
    case 20:  return HE_OPER_CHANWIDTH_20;	
    case 40:  return HE_OPER_CHANWIDTH_40;
    case 80:  return HE_OPER_CHANWIDTH_80;
    case 160: return HE_OPER_CHANWIDTH_160;
    default: wpa_printf(MSG_ERROR, "ACS: wrong or unsupported channel width %d", width);
  }
...
static void acs_smart_adjust_vht_center_freq(struct hostapd_iface *iface, unsigned center_chan_index)
{
	...
	hostapd_update_op_class(iface->conf);
}

static void acs_smart_adjust_6g_center_freq(struct hostapd_iface *iface, unsigned center_chan_index)
{
	int seg0_160 = 0;
	int chwidth = hostapd_get_oper_chwidth(iface->conf);
	...
	switch (chwidth) {
		...
		case HE_OPER_CHANWIDTH_160:
			hostapd_set_oper_centr_freq_seg1_idx(iface->conf, center_chan_index);
			seg0_160 = (iface->conf->channel > center_chan_index ? (center_chan_index + CF0_AND_CF1_IDX_DIFF_IN_160_MHZ)
									     : (center_chan_index - CF0_AND_CF1_IDX_DIFF_IN_160_MHZ));
			hostapd_set_oper_centr_freq_seg0_idx(iface->conf, seg0_160);
			break;
		default:
			wpa_printf(MSG_ERROR, "ACS SMART 6GHz: invalid channel width");
			return;
	}
	iface->conf->op_class = HOSTAP_6GHZ_20MHZ_OP_CLASS + chwidth;
	iface->conf->he_oper.vht_op_info_chwidth = chwidth;
	iface->conf->he_oper.vht_op_info_chan_center_freq_seg0_idx = hostapd_get_oper_centr_freq_seg0_idx(iface->conf);
	iface->conf->he_oper.vht_op_info_chan_center_freq_seg1_idx = hostapd_get_oper_centr_freq_seg1_idx(iface->conf);

	wpa_printf(MSG_DEBUG, "ACS SMART 6GHz: op_clas = %d, chwidth = %d, seg0 = %d, seg1 = %d",
				iface->conf->op_class, chwidth,
...
void acs_smart_record_bsses(struct hostapd_iface *iface, struct wpa_scan_results *scan_res, FILE *fp)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (is_6ghz_op_class(iface->conf->op_class)) {
			he_oper = (struct ieee80211_he_operation *)elems.pc_he_operation;

			if (he_oper && (he_oper->he_oper_params[HE_OPERATION_CAP2_IDX] & HE_OPERATION_CAP2_6GHZ_OPERATION_INFO_PRESENT)
									&& (!(he_oper->he_oper_params[HE_OPERATION_CAP1_IDX] &
										 (HE_OPERATION_CAP1_CO_LOCATED_BSS | HE_OPERATION_CAP1_VHT_OPER_INFO_PRESENT)))) {
				he_6ghz_oper_info = &(he_oper->vht_op_info_chwidth);
				he_6ghz_pri_chan = he_6ghz_oper_info[HE_OPER_6GHZ_INFO_CHANNEL_OFFSET];
				he_6ghz_seg0_offset = he_6ghz_oper_info[HE_OPER_6GHZ_INFO_SEG0_OFFSET];
				he_6ghz_seg1_offset = he_6ghz_oper_info[HE_OPER_6GHZ_INFO_SEG1_OFFSET];
				bss_width = he_6ghz_oper_info[HE_OPER_6GHZ_INFO_CONTROL_OFFSET] & HE_OPERATION_6GHZ_CONTROL_CHANNEL_WIDTH;
				bss_center_freq0 = STARTING_6G_FREQ_IN_MHZ + (5 * he_6ghz_seg0_offset);
				if (he_6ghz_seg1_offset)
					bss_center_freq1 = STARTING_6G_FREQ_IN_MHZ + (5 * he_6ghz_seg1_offset);
		...
		if (elems.rnr) {
			ieee80211_parse_rnr(&elems, &iface->interfaces->rnr_scan_6g_ap_info);
		}
...
static void acs_scan_complete(struct hostapd_iface *iface)
{
	...
	wpa_printf(MSG_DEBUG, "ACS: acs_scan_complete for op_class : %d", iface->conf->op_class);

	if (is_6ghz_op_class(iface->conf->op_class)) {

		if (iface->acs_6g_scan_type == SCAN_6G_FULL_BAND ||
			iface->acs_6g_scan_type == SCAN_6G_RNR_PASSIVE)
			iface->acs_6g_scan_status = SCAN_6G_COMPLETE;

		if (iface->acs_6g_scan_status != SCAN_6G_COMPLETE) {
			iface->acs_6g_scan_status = SCAN_6G_DONE;
			acs_update_scan_states(iface);
			return;
		}
	}
	...
	if (!(is_6ghz_op_class(iface->conf->op_class))) {
		for (count = 0; count < iface->interfaces->count; count++) {
			if (is_6ghz_op_class(iface->interfaces->iface[count]->conf->op_class))	{
				...
				if (iface->interfaces->iface[count]->acs_6g_scan_type == SCAN_6G_RNR_PASSIVE &&
					iface->interfaces->iface[count]->acs_6g_scan_status == SCAN_6G_WAITING) {
					acs_rnr_scan_cb(iface->interfaces->iface[count]);
				}
			}
		}
	}
	...
	if (is_6ghz_op_class(iface->conf->op_class)) {
		acs_restore_6g_scan_states(iface);
	}
...
const char * acs_6g_scan_state_text(enum hostapd_acs_6g_scan_status status)
{
	switch (status) {
...
const char * acs_6g_scan_type_text(enum hostapd_acs_6g_scan_type type)
{
	switch (type) {
...
void acs_get_rnr_status(struct hostapd_iface *iface, u8 *rnr_24g, u8 *rnr_5g)
{
	...
	for (count = 0; count < iface->interfaces->count; count++) {
		if (is_24ghz_op_class(iface->interfaces->iface[count]->conf->op_class)) {
			*rnr_24g = iface->interfaces->iface[count]->acs_rnr_available;
		}
		else if (is_5ghz_op_class(iface->interfaces->iface[count]->conf->op_class)) {
			*rnr_5g = iface->interfaces->iface[count]->acs_rnr_available;
		}
	}
}

void acs_rnr_scan_timer_handler(void *eloop_ctx, void *timeout_ctx)
{
	...
	acs_get_lower_band_status(iface);

	if ((iface->acs_6g_scan_type == SCAN_6G_RNR_PASSIVE ||
		iface->acs_6g_scan_type == SCAN_6G_FULL_BAND) &&
		(iface->acs_6g_scan_status == SCAN_6G_STARTED)) {
...
void acs_init_defaults_candidate_table(struct hostapd_iface *iface)
{
	...
	for (i = 0; (i < iface->num_candidates && i < MAX_CANDIDATES); i++) {
		if (is_6ghz_psc_freq(candidates[i].freq)) {
	...
	for (i = 0; (i < iface->num_candidates && i < MAX_CANDIDATES); i++) {
		...
		if (is_6ghz_psc_freq(candidates[i].freq) || is_6ghz_opt_freq(iface, candidates[i].freq))
			continue;
...
void acs_restore_6g_scan_states(struct hostapd_iface *iface)
{
	iface->acs_6g_scan_type = SCAN_6G_NONE;
	iface->acs_6g_scan_status = SCAN_6G_IDLE;
}

void acs_get_lower_band_status(struct hostapd_iface *iface)
{
	...
	if (rnr_24g || rnr_5g) {
		acs_6ghz_prepare_rnr_scan_data(iface);
		acs_init_defaults_candidate_table(iface);
		if (iface->acs_6ghz_passive_ch_num) {
			iface->acs_6g_scan_type = SCAN_6G_FULL_BAND;
			iface->acs_6g_scan_status = SCAN_6G_STARTED;
		}
		else {
...
void acs_update_scan_states(struct hostapd_iface *iface)
{

	if (iface->acs_6g_scan_type == SCAN_6G_NONE &&
		iface->acs_6g_scan_status == SCAN_6G_IDLE) {

		/* Looks to be the first scan call for 6G */
		iface->acs_6g_scan_type = SCAN_6G_PSC_OPT;
		iface->acs_6g_scan_status = SCAN_6G_STARTED;
		wpa_printf(MSG_DEBUG, "6G ACS - Update : scan_type : %s->%s  scan_state : %s->%s",
					acs_6g_scan_type_text(SCAN_6G_NONE), acs_6g_scan_type_text(iface->acs_6g_scan_type),
					acs_6g_scan_state_text(SCAN_6G_IDLE), acs_6g_scan_state_text(iface->acs_6g_scan_status));
		return;
	} else if (iface->acs_6g_scan_type == SCAN_6G_PSC_OPT && iface->acs_6g_scan_status == SCAN_6G_DONE) {

		/* PSC Scan is completed and Next is RNR */
		wpa_printf(MSG_DEBUG, "6G ACS PSC is completed - waiting for lower bands RNR results ...");
		iface->acs_6g_scan_type = SCAN_6G_RNR_PASSIVE;
		iface->acs_6g_scan_status = SCAN_6G_WAITING;
		eloop_register_timeout(iface->conf->acs_rnr_scan_timeout, 0, acs_rnr_scan_timer_handler, iface, NULL);
	} else {
		wpa_printf(MSG_ERROR, "6G ACS - Invalid scan state - Update : scan_type : %s scan_state : %s",
					acs_6g_scan_type_text(iface->acs_6g_scan_type),
					acs_6g_scan_state_text(iface->acs_6g_scan_status));
		goto complete;
	}
	return;
complete:
...
void acs_rnr_scan_cb(struct hostapd_iface *iface)
{
	...
	if (rnr_24g && rnr_5g) {
		...
		eloop_cancel_timeout(acs_rnr_scan_timer_handler, iface, NULL);
		acs_6ghz_prepare_rnr_scan_data(iface);
		acs_init_defaults_candidate_table(iface);
...
static void acs_init_candidate_table(struct hostapd_iface *iface)
{
	...
	for (i = 0; i < mode->num_channels; i++) {
		...
		if ((!iface->conf->secondary_channel && !is_6ghz_op_class(iface->conf->op_class)) ||
			(is_6ghz_op_class(iface->conf->op_class) && (hostapd_get_oper_chwidth(iface->conf) == HE_OPER_CHANWIDTH_20))) { /* 40 MHz not supported */
			continue;
		}
		...
		if (chan->freq < 5000) { /* 2.4 GHz */
		...
		} else if (chan->freq < STARTING_6G_FREQ_IN_MHZ) { /* 5 GHz */
			...
			add_channel_pair_candidates(iface, g_channel_pairs_40_6G, sizeof(g_channel_pairs_40_6G) / sizeof(channel_pair),
										STARTING_6G_FREQ_IN_MHZ, chan);
			if (hostapd_get_oper_chwidth(iface->conf) >= HE_OPER_CHANWIDTH_80) { /* 80 MHz supported */
				add_80_channels(iface, g_channels80_6G, sizeof(g_channels80_6G) / sizeof(channel_80), chan);
			}
			if (hostapd_get_oper_chwidth(iface->conf) == HE_OPER_CHANWIDTH_160) { /* 160 MHz supported */
				add_160_channels(iface, g_channels160_6G, sizeof(g_channels160_6G) / sizeof(channel_160), chan);
			}
		}
	}
...
int is_6ghz_opt_freq(struct hostapd_iface *iface, int freq)
{
	int count;

	for(count = 0; count < iface->conf->acs_6g_opt_ch_count ; count++) {
		if (freq == (hostapd_chan_to_freq(iface->conf->acs_6g_opt_ch_list[count], iface->conf->op_class)))
			return 1;
	}
	return 0;
}

int is_6ghz_rnr_passive_scan_freq(struct hostapd_iface *iface, int freq)
{
	int ch;

	for (ch = 0; ch < iface->acs_6ghz_passive_ch_num; ch++) {
		if (freq == iface->acs_6ghz_passive_ch_list[ch])
			return 1;
	}
	return 0;
}

void acs_6ghz_prepare_rnr_scan_data(struct hostapd_iface *iface)
{
	struct hapd_interfaces *ifaces = iface->interfaces;
	...
	dl_list_for_each(scan_ap_info, &ifaces->rnr_scan_6g_ap_info, rnr_scan_ap_info_t, list) {
		if (scan_ap_info->flag & RNR_AOCS_SCAN_PASSIVE_CHANNEL) {
			if (is_6ghz_psc_freq(scan_ap_info->freq) || is_6ghz_opt_freq(iface, scan_ap_info->freq))
				continue;
			else
				iface->acs_6ghz_passive_ch_list[passive_ch++] = scan_ap_info->freq;
		}
	...
	iface->acs_6ghz_passive_ch_num = passive_ch;
	wpa_printf(MSG_DEBUG, "6G ACS : RNR Scan - No. of Passive channels discovered %d", iface->acs_6ghz_passive_ch_num);
}
...
static int acs_request_scan(struct hostapd_iface *iface)
{
	...
	for (i = 0; i < iface->current_mode->num_channels; i++) {
		...
		if (is_6ghz_op_class(iface->conf->op_class))
		{
			switch (iface->acs_6g_scan_type) {
				case SCAN_6G_PSC_OPT:
					/* 6G PSC or OPT list */
					if (!(is_6ghz_psc_freq(chan->freq) || is_6ghz_opt_freq(iface, chan->freq)))
						continue;
					break;
				...
				case SCAN_6G_RNR_PASSIVE:
					...
					if (!is_6ghz_rnr_passive_scan_freq(iface, chan->freq))
						continue;
					break;
...
int acs_set_new_chan_if_ok(struct hostapd_iface *iface, int min_rank_cand_idx, int switch_reason, FILE *fp_hist)
{
	...
	actual_width     = get_num_width(hostapd_get_oper_chwidth(iface->conf), actual_secondary, is_6ghz_op_class(iface->conf->op_class));
	...
	if ((iface->selected_candidate < 0) || /* if previously selected candidate is not known - switch */
		...
		if ((candidates[min_rank_cand_idx].primary   != actual_primary) ||
			...
			iface->selected_candidate = min_rank_cand_idx;
			if (is_6ghz_op_class(iface->conf->op_class)) {
				hostapd_set_oper_chwidth(iface->conf, get_he_width(candidates[min_rank_cand_idx].width));
				acs_smart_adjust_6g_center_freq(iface, candidates[min_rank_cand_idx].chan);
			} else {
...
static void acs_print_timeout_and_degradation_factors(int *acs_to_degradation)
{
	...
	for (i = 0; i < ACS_NUM_DEGRADATION_FACTORS; i++) {
		wpa_printf(MSG_DEBUG, "\t%d: %s factor %d", i, acs_norm_to_degr_to_str(i), *acs_to_degradation);
		acs_to_degradation++;
	}
}
						   ...
						   int chan, int load, int cwi)
{
	...
	for (i = 0; (i < iface->num_candidates && i < MAX_CANDIDATES); i++) {
		...
		if (channels_overlap(hostapd_chan_to_freq(chan, is_6ghz_op_class(iface->conf->op_class)), 20, candidates[i].freq, candidates[i].width)) {
...
set_chan:
		...
		if (res) {
			int width = get_num_width(hostapd_get_oper_chwidth(iface->conf), candidates[min_rank_cand_idx].secondary,
	...
	if (!is_6ghz_op_class(iface->conf->op_class)) {
		...
		hostapd_set_oper_chwidth(iface->conf,get_he_width(iface->conf->acs_fallback_chan.width));
		acs_smart_adjust_6g_center_freq(iface, iface->conf->channel);
	}
