...
int wpa_init_bigtk_keys(struct wpa_authenticator *wpa_auth)
{
	struct wpa_group *group = wpa_auth->group;

	wpa_printf(MSG_ERROR, "WPA: Start group state machine to set initial "
			"beacon protection keys");

	group->GInit = false;
	group->BP_GInit = true;
	wpa_group_sm_step(wpa_auth, group);
	group->BP_GInit = false;
	wpa_group_sm_step(wpa_auth, group);
	group->GInit = true;
...
static int ieee80211w_kde_len(struct wpa_state_machine *sm, bool igtk, bool bigtk)
{
	if (!igtk && !bigtk)
		return 0;
	if (igtk && !sm->mgmt_frame_prot)
		return 0;
...
static u8 * ieee80211w_bigtk_kde_add(struct wpa_state_machine *sm, u8 *pos)
{
	struct wpa_igtk_kde bigtk;
	...
	os_memcpy(bigtk.igtk, gsm->BIGTK[gsm->GN_bigtk - 6], len);
	pos = wpa_add_kde(pos, RSN_KEY_DATA_BIGTK,
				(const u8 *) &bigtk, WPA_IGTK_KDE_PREFIX_LEN + len,
				NULL, 0);
	return pos;
}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	kde_len = wpa_ie_len + ieee80211w_kde_len(sm, 1, 0) + ocv_oci_len(sm);
	if (sm->wpa_auth->conf.beacon_protection_enabled)
		kde_len += ieee80211w_kde_len(sm, 0, 1) + ocv_oci_len(sm);
	if (gtk)
	...
	if (ocv_oci_add(sm, &pos, sm->wpa_auth->conf.oci_freq_override_eapol_m3) < 0)
		goto done;

	if (sm->wpa_auth->conf.beacon_protection_enabled) {
		pos = ieee80211w_bigtk_kde_add(sm, pos);
		if (ocv_oci_add(sm, &pos,sm->wpa_auth->conf.oci_freq_override_eapol_m3) < 0) {
			os_free(kde);
			return;
		}
...
SM_STATE(WPA_PTK_GROUP, REKEYNEGOTIATING)
{
	...
	if (sm->wpa == WPA_VERSION_WPA2) {
		kde_len = 2 + RSN_SELECTOR_LEN + 2 + gsm->GTK_len +
			ieee80211w_kde_len(sm, 1, 0) + ocv_oci_len(sm);
		if (sm->wpa_auth->conf.beacon_protection_enabled)
			kde_len += ieee80211w_kde_len(sm, 0, 1) + ocv_oci_len(sm);
		...
		if (ocv_oci_add(sm, &pos,
				sm->wpa_auth->conf.oci_freq_override_eapol_g1) < 0) {
			os_free(kde_buf);
			return;
		}
		if (sm->wpa_auth->conf.beacon_protection_enabled) {
			pos = ieee80211w_bigtk_kde_add(sm, pos);
			if (ocv_oci_add(sm, &pos, sm->wpa_auth->conf.oci_freq_override_eapol_g1) < 0) {
				os_free(kde_buf);
				return;
			}
		}
...
static int wpa_bigtk_update(struct wpa_authenticator *wpa_auth,
				struct wpa_group *group)
{
	int ret = 0;
	size_t len;
	static bool sbigtk_gen_done = 0;
	static int sbigtk_curr_key_idx = 6;
	static u8 SBIGTK[WPA_IGTK_MAX_LEN];

	len = wpa_cipher_key_len(wpa_auth->conf.group_mgmt_cipher);
	os_memcpy(group->GNonce, group->Counter, WPA_NONCE_LEN);
	inc_byte_array(group->Counter, WPA_NONCE_LEN);

	if (sbigtk_curr_key_idx != group->GN_bigtk) {
		sbigtk_gen_done = 0;
		sbigtk_curr_key_idx = group->GN_bigtk;
	}

	if (wpa_auth->conf.multibss_enable && sbigtk_gen_done) {
		os_memcpy(group->BIGTK[group->GN_bigtk - 6], SBIGTK, len);
	} else if (wpa_gmk_to_gtk(group->GMK, "BIGTK key expansion",
				  wpa_auth->addr, group->GNonce,
	...
	if (wpa_auth->conf.multibss_enable && !sbigtk_gen_done)
		os_memcpy(SBIGTK, group->BIGTK[group->GN_bigtk - 6], len);

	if (wpa_auth->conf.multibss_enable)
		sbigtk_gen_done = 1;
...
static void wpa_group_bigtk_init(struct wpa_authenticator *wpa_auth,
				 struct wpa_group *group)
{
	wpa_printf(MSG_ERROR, "WPA: group state machine entering state "
			"BIGTK_INIT (VLAN-ID %d)", group->vlan_id);

	group->wpa_group_state = WPA_GROUP_BIGTK_INIT;

	/* BIGTK[0..N] = 0 */
	os_memset(group->BIGTK, 0, sizeof(group->BIGTK));
	group->GN_bigtk = 6;
	group->GM_bigtk = 7;
	wpa_bigtk_update(wpa_auth, group);
}
			      ...
			      struct wpa_group *group)
{
	...
	wpa_gtk_update(wpa_auth, group);
	if (wpa_auth->conf.beacon_protection_enabled &&
		!wpa_auth->conf.disable_bigtk_rekey) {
		tmp = group->GM_bigtk;
...
static int wpa_group_bigtk_setkeysdone(struct wpa_authenticator *wpa_auth,
					struct wpa_group *group)
{
	if (!wpa_auth->conf.beacon_protection_enabled)
		return 0;

	if (wpa_auth->conf.disable_bigtk_rekey &&
	    group->wpa_group_state == WPA_GROUP_SETKEYSDONE)
		return 0;

	wpa_printf(MSG_ERROR, "WPA: group state machine entering state "
			"BIGTK SETKEYSDONE (VLAN-ID %d)", group->vlan_id);
	...
	if (wpa_auth_set_key(wpa_auth, group->vlan_id, alg,
				...
				group->BIGTK[group->GN_bigtk - 6], len, KEY_FLAG_GROUP_TX_DEFAULT))
		return -1;
			      ...
			      struct wpa_group *group)
{
	if (group->GInit) {
		...
		if (group->GKeyDoneStations == 0) {
			wpa_group_setkeysdone(wpa_auth, group);
			wpa_group_bigtk_setkeysdone(wpa_auth, group);
		} else if (group->GTKReKey) {
			wpa_group_setkeys(wpa_auth, group);
		}
	} else if (group->BP_GInit) {
		wpa_group_bigtk_init(wpa_auth, group);
	} else if (group->wpa_group_state == WPA_GROUP_BIGTK_INIT) {
		wpa_group_bigtk_setkeysdone(wpa_auth, group);
	}
}
