...
#ifdef CONFIG_IEEE80211AX
	/*Broadcast TWT default configuration*/
	conf->btwt_wake_dur_unit = TWT_WAKE_DURATION_UNIT_256US;
#endif
...
#define DEFAULT_RNR_SCAN_TIMEOUT 2 /* secs */
	conf->acs_rnr_scan_timeout = DEFAULT_RNR_SCAN_TIMEOUT;
...
continue_no_fail:
	...
	conf->he_mu_edca.he_mu_ac_be_param[HE_MU_AC_PARAM_MU_EDCA_TIMER_IDX] = 5;
	...
	conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_ECWMIN_ECWMAX_IDX] |=
									set_he_cap(15, HE_MU_AC_PARAM_ECWMAX);
	conf->he_mu_edca.he_mu_ac_bk_param[HE_MU_AC_PARAM_MU_EDCA_TIMER_IDX] = 5;
	...
	conf->he_mu_edca.he_mu_ac_vi_param[HE_MU_AC_PARAM_MU_EDCA_TIMER_IDX] = 5;
	...
	conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_ECWMIN_ECWMAX_IDX] |=
								set_he_cap(15, HE_MU_AC_PARAM_ECWMAX);
	conf->he_mu_edca.he_mu_ac_vo_param[HE_MU_AC_PARAM_MU_EDCA_TIMER_IDX] = 5;
...
void hostapd_config_free(struct hostapd_config *conf)
{
	...
	os_free(conf->acs_6g_opt_ch_list);
#endif /* CONFIG_ACS */
...
int hostapd_config_check(struct hostapd_config *conf, int full_config)
{
	...
	if (full_config && conf->ieee80211d &&
	    !conf->bss[0]->ssid.request_credentials &&
