...
static int ieee80211n_is_allowed_ht40_channel(struct hostapd_iface *iface, int channel)
{
	...
	modes = iface->hw_features;
	num_modes = iface->num_hw_features;
...
static void ieee80211n_check_scan(struct hostapd_iface *iface)
{
	...
	if (iface->conf->secondary_channel &&
		...
		he_cap = &iface->current_mode->he_capab;
		if (!(he_cap->he_phy_capab_info[HE_PHYCAP_CAP0_IDX] &
		      HE_PHY_CAP0_CHANNEL_WIDTH_SET_B0)) {
			wpa_printf(MSG_DEBUG,
				   "HE: 40 MHz channel width is not supported in 2.4 GHz; "
				   "clear secondary channel configuration");
			iface->conf->secondary_channel = 0;
		}
	}
					...
					struct wpa_driver_scan_params *params)
{
  ...
  /* If VHT channel used, trigger scan for all channels in chanwidth to calibrate all those channels */
	if ((iface->conf->ieee80211ac || iface->conf->ieee80211ax) && hostapd_get_oper_chwidth(iface->conf) == CHANWIDTH_80MHZ) {
		int low_freq = hostapd_hw_get_freq(iface->bss[0], hostapd_get_oper_centr_freq_seg0_idx(iface->conf) - 6);
...
int hostapd_check_he_6ghz_capab(struct hostapd_iface *iface)
{
	...
	hw = iface->current_mode->he_6ghz_capab.capab;
...
static int hostapd_is_usable_chans(struct hostapd_iface *iface)
{
	...
	if (!iface->conf->ht40_plus_minus_allowed
	    ...
	    || ((iface->conf->acs_algo == ACS_ALGO_SMART) && iface->conf->acs_init_done)
#endif
	    )
...
static void hostapd_notify_bad_chans(struct hostapd_iface *iface)
{
	...
	hostapd_logger(iface->bss[0], NULL,
		       ...
		       HOSTAPD_LEVEL_WARNING,
		       "Configured channel (%d) or frequency (%d) (secondary_channel=%d) "
		       "not found from the channel list of the current mode (%d) %s",
		       iface->conf->channel,
...
#ifdef CONFIG_ACS
int hostapd_acs_completed(struct hostapd_iface *iface, int err, int set_channel)
{
	int ret = -1;
	int dfs_channel = 0;
	struct hostapd_channel_data* chan;
	...
	if (!iface->conf->acs_scan_mode) {
			wpa_msg(iface->bss[0]->msg_ctx, MSG_INFO,
					...
					get_num_width(hostapd_get_oper_chwidth(iface->conf), iface->conf->secondary_channel,
	 ...
	 */
	ch_width = get_num_width(hostapd_get_oper_chwidth(iface->conf), iface->conf->secondary_channel,
							 is_6ghz_op_class(iface->conf->op_class));
	if (WAVE_FREQ_IS_5G(chan->freq) || WAVE_FREQ_IS_6G(chan->freq)) {
		if (ch_width < CHWIDTH_160) {
 ...
 * @check_chans: Flag to check/validate currently configured channels
 * Returns: 0 on success, < 0 on failure
 *
 * Sets up the hardware mode, channel, rates, and passive scanning
 * based on the configuration.
 */
int hostapd_select_hw_mode(struct hostapd_iface *iface, int check_chans)
{
	int i;
	...
	if (hostapd_send_dfs_channel_state(iface) < 0)
		return -1;
