...
channel_80 g_channels80_6G[NUM_CHAN_PAIRS_80_6G] =
{
...
channel_160 g_channels160_6G[NUM_CHAN_PAIRS_160_6G] =
{
...
#define BTWT_DEFAULT_PERSISTENCE_VALUE 5
	...
	if ((hapd->conf->bcast_twt_config_enable) &&
	    ...
	    (mode->he_capab.he_mac_capab_info[HE_MACCAP_CAP0_IDX] & HE_MAC_CAP0_TWT_RESPONDER_SUPPORT) &&
	    (mode->he_capab.he_mac_capab_info[HE_MACCAP_CAP2_IDX] & HE_MAC_CAP2_BROADCAST_TWT_SUPPORT)) {
		return true;
...
static void hostapd_post_up_vendor_vap_cmd(struct hostapd_data *hapd, struct hostapd_bss_config *conf)
{
	...
	if(hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_UDMA_API,
			(u8*)&conf->sUdmaEnabled, sizeof(conf->sUdmaEnabled),
			NESTED_ATTR_NOT_USED, NULL))
	...
	if(conf->sUdmaEnabled){
		if(hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_UDMA_VLAN_ID,
				(u8*)&conf->sUdmaVlanId, sizeof(conf->sUdmaVlanId), NESTED_ATTR_NOT_USED, NULL))
	...
	if (is_6ghz_op_class(hapd->iconf->op_class) && 
		...
		(hapd->iconf->multibss_enable))) {
			...
			/* TODO: If unsolicited frames are disabled and legacy band interfaces go down,
			 *       we may need to detect such scenarios and enable UPR or FILS frames.
			 */
...
static int hostapd_pre_up_vendor_vap_cmd(struct hostapd_data *hapd)
{
	...
	u8 he_oper_cap2_ersu_disable = get_he_cap(hapd->iface->conf->he_oper.he_oper_params[HE_OPERATION_CAP2_IDX], HE_OPERATION_CAP2_ER_SU_DISABLE);
	u8 he_oper_cap2_ersu_disable_override = hapd->iface->conf->he_oper_er_su_disable_override;
	...
	if (hostapd_drv_get_he_operation(hapd, &he_operation_hw) < 0) {
		...
		if (hapd->iconf->num_bss > MIN_NUM_BSS_IN_NON_MBSSID_SET && !hapd->iconf->multibss_enable) {
			clr_set_he_cap(&he_operation_hw.he_oper_params[HE_OPERATION_CAP1_IDX],
							1, HE_OPERATION_CAP1_CO_LOCATED_BSS);
		} else {
			clr_set_he_cap(&he_operation_hw.he_oper_params[HE_OPERATION_CAP1_IDX],
							0, HE_OPERATION_CAP1_CO_LOCATED_BSS);
		}

		if (he_oper_cap2_ersu_disable_override)
			clr_set_he_cap(&he_operation_hw.he_oper_params[HE_OPERATION_CAP2_IDX],
					he_oper_cap2_ersu_disable, HE_OPERATION_CAP2_ER_SU_DISABLE);
...
#ifdef CONFIG_IEEE80211AX
void hostapd_send_btwt_configuration(struct hostapd_data *hapd)
{
...
int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
	...
	if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_SOFTBLOCK_DISABLE,
			...
			sizeof(hapd->conf->sDisableSoftblock), NESTED_ATTR_NOT_USED, NULL))
	...
	if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_SB_TIMER_ACL_CFG,
			(u8*)&sb_timer_acl_cfg, sizeof(sb_timer_acl_cfg), NESTED_ATTR_NOT_USED, NULL))
	...
	if (hapd->conf->sBridgeMode != -1){
		if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_BRIDGE_MODE,
				...
				sizeof(hapd->conf->sBridgeMode), NESTED_ATTR_NOT_USED, NULL))
	...
	if (hapd->conf->num_sAddPeerAP > 0 && hapd->conf->sAddPeerAP != NULL)
	{
		...
		for (i=0; i < hapd->conf->num_sAddPeerAP; i++)
		{
			...
			if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_ADD_PEERAP,
					(u8*)data8, sizeof(data8), NESTED_ATTR_NOT_USED, NULL))
...
static int setup_interface(struct hostapd_iface *iface)
{
	...
	/*
	 * when mem_only_cred configuration is set we will halt setup interface
	 * and request credentials from the UI
	 */
...
#define FILS_OPTIONAL_ELEMENTS_SIZE     (FILS_OP_CLASS_FIELD_SIZE +\
	...
	if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SEND_FILS_DISCOVERY_FRAME,
						(const u8*)buf, buflen, NESTED_ATTR_NOT_USED, NULL)) {
...
static void hostapd_pre_up_vendor_radio_cmd(struct hostapd_iface *iface)
{
	...
	if (iface->allow_scan_during_cac == -1) {
		if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_ALLOW_SCAN_DURING_CAC,
					   (u8*)&iface->conf->allow_scan_during_cac, sizeof(iface->conf->allow_scan_during_cac),
					   NESTED_ATTR_NOT_USED, NULL))
...
static void hostapd_post_up_vendor_radio_cmd(struct hostapd_iface *iface)
{
	...
	if (hostapd_drv_vendor_cmd(hapd, OUI_LTQ, LTQ_NL80211_VENDOR_SUBCMD_SET_NFRP_CFG,
				   (u8*)&nfrpcfg[0], sizeof(nfrpcfg), NESTED_ATTR_NOT_USED, NULL))
						 ...
						 int err)
{
	...
	hostapd_pre_up_vendor_radio_cmd(iface);
		...
		struct hostapd_config *new_conf, int new_bss_idx)
{
	...
	if (iface->num_bss > MIN_NUM_BSS_IN_NON_MBSSID_SET) {
		/* Update beacon for all VAPS with co_hosted bit set */
...
static int hostapd_remove_bss(struct hostapd_iface *iface, unsigned int idx)
{
	...
	if (iface->conf->num_bss == MIN_NUM_BSS_IN_NON_MBSSID_SET) {
		for (i = 0; i < iface->conf->num_bss; i++) {
			hapd = iface->bss[i];
			clr_set_he_cap(&iface->conf->he_oper.he_oper_params[HE_OPERATION_CAP1_IDX],
							0, HE_OPERATION_CAP1_CO_LOCATED_BSS);
			ieee802_11_update_beacon(hapd);
		}
	}
			   ...
			   struct csa_settings *settings)
{
	...
	* */
	if (hapd->conf->start_disabled) {
		bss = hapd->drv_priv;
		bss->freq = settings->freq_params.freq;
		return 0;
	}
...
int hostapd_get_secondary_offset(int primary, int bandwidth, int center_idx)
{
	if (bandwidth == 20)
		return 0;
...
hostapd_set_conf_from_freq_params(struct hostapd_iface *iface, const struct hostapd_freq_params *freq_params)
{
	...
	if (freq_params->bandwidth > 20 && iface->conf->secondary_channel == 0)
		iface->conf->secondary_channel = hostapd_get_secondary_offset(iface->conf->channel, freq_params->bandwidth, bw_center_idx);
				...
				const struct hostapd_freq_params *freq_params)
{
	...
	hostapd_set_conf_from_freq_params(iface, freq_params);
...
int get_num_width(int width, int secondary, bool op_class_6g)
{
	if (op_class_6g) {
	  switch (width) {
		case HE_OPER_CHANWIDTH_20 : return 20;
		case HE_OPER_CHANWIDTH_40 : return 40;
		case HE_OPER_CHANWIDTH_80 : return 80;
		case HE_OPER_CHANWIDTH_160: return 160;
		default: 
			wpa_printf(MSG_ERROR, "ACS: wrong or unsupported channel HE width %d", width);
			break;
	  }
	} else {
	  switch (width) {
		...
		default: wpa_printf(MSG_ERROR, "ACS: wrong or unsupported channel VHT width %d", width);
	  }
	}
...
/* Helper function to update op_class for 2.4G and 5G band
 * based on channel and channel_width
 */
void hostapd_update_op_class (struct hostapd_config *conf)
{
	...
	switch (hostapd_get_oper_chwidth(conf)) {
		case CHANWIDTH_USE_HT:
			/* 2.4GHz */
			if (conf->channel >= 1 && conf->channel <= 13) {
				conf->op_class = 81;
			} else if (conf->channel == 14) {
				conf->op_class = 82;
			/* 5GHz */
			} else if (conf->channel >= 36 && conf->channel <= 64) {
				conf->op_class = 115;
			} else if (conf->channel >= 100 && conf->channel <= 140) {
				conf->op_class = 121;
			} else if (conf->channel >= 149 && conf->channel <= 161) {
				conf->op_class = 124;
			} else if (conf->channel > 161 && conf->channel <= 169) {
				conf->op_class = 125;
			}
			break;
		case CHANWIDTH_80MHZ:
			if (conf->channel >= 36 && conf->channel <= 161)
				conf->op_class = 130;
			break;
		case CHANWIDTH_160MHZ:
			if (conf->channel >= 36 && conf->channel <= 128)
				conf->op_class = 129;
			break;
		default:
			break;
