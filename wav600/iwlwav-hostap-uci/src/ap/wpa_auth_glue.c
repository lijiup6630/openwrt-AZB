				  ...
				  struct wpa_auth_config *wconf)
{
	...
	wconf->beacon_protection_enabled = conf->beacon_protection_enabled;
	wconf->disable_bigtk_rekey = conf->disable_bigtk_rekey;
	wconf->multibss_enable = iconf->multibss_enable;
				    ...
				    size_t key_len, enum key_flag key_flag)
{
	...
	return hostapd_drv_set_key(ifname, hapd, alg, addr, idx, 0, 1, NULL, 0,
...
int hostapd_setup_wpa(struct hostapd_data *hapd)
{
	...
	if (!(hapd->iface->drv_flags & WPA_DRIVER_FLAGS_BEACON_PROTECTION))
		_conf.beacon_protection_enabled = 0;
