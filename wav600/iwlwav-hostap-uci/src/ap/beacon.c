		      ...
		      int ssi_signal, int snr_db)
{
	...
	if (hapd->iconf->rssi_ignore_probe_request && ssi_signal &&
		...
		wpa_printf(MSG_DEBUG, "Ignoring probe request rssi_ignore_probe_request=%d ssi_signal=%d",
			   hapd->iconf->rssi_ignore_probe_request, ssi_signal);
		return;
	}
...
static int get_psd_val(struct hostapd_iface *iface, u8 *val)
{
	...
	if (hostapd_drv_get_tx_power_20mhz(hapd, &tx_power_20mhz)) {
		wpa_printf(MSG_INFO, "failed to get tx_power_20mhz : %d", tx_power_20mhz);
		return -1;
	}
...
int auto_update_rnr_in_beacon(struct hostapd_iface *iface)
{
	...
	(six_ghz_iface_idx < MAX_SUPPORTED_6GHZ_RADIO); i++) {
		...
		if (!iface->conf->channel) {
			wpa_printf(MSG_INFO, "%s:%d ACS running on radio %zd, skip RNR update",
				   __func__, __LINE__, i);
			return 0;
		}
...
int ieee802_11_set_beacon(struct hostapd_data *hapd)
{
	...
	/* if we enable the VAP interface through hostapd_cli
	 * start_disbaled command the bss->freq should be updated
	 * here so that VAP will be in sync with Radio.
	 * */
	bss = hapd->drv_priv;
	bss->freq = iface->freq;
