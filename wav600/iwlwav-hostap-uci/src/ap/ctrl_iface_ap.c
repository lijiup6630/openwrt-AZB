				      ...
				      char *buf, size_t buflen)
{
	...
	if ((sta->flags & WLAN_STA_HE) && sta->he_capabilities) {
		...
		u8 he_mcs_nss_size = MIN_HE_MCS_NSS_LEN , he_ppe_size = 0;

		he = (u8*) &sta->he_capabilities->he_mac_capab_info;
		ret = sprintf_s(buf + len, buflen - len, "he_mac=");
		if (ret <= 0)
			return len;
		len += ret;
		for (i = 0; i < sizeof(sta->he_capabilities->he_mac_capab_info); i++) {
			ret = sprintf_s(buf + len, buflen - len, "%02x%s", *(he + i),
		...
		ret = sprintf_s(buf + len, buflen - len, "he_phy=");
		if (ret <= 0)
			return len;
		len += ret;
		...
		he_mcs_nss_size = get_he_mcs_nss_size((u8 *)sta->he_capabilities);

		he = (u8*) &sta->he_capabilities->optional;
		ret = sprintf_s(buf + len, buflen - len, "he_mcs_nss=");
		if (ret <= 0)
			return len;
		len += ret;
		for (i = 0; i < he_mcs_nss_size; i++) {
			ret = sprintf_s(buf + len, buflen - len, "%02x ", *(he + i));
			if (ret <= 0)
				return len;
			len += ret;
		}
		...
		he += he_mcs_nss_size;
		ret = sprintf_s(buf + len, buflen - len, "he_ppe_thresholds=");
		if (ret <= 0)
			return len;
		len += ret;

		he_ppe_size = get_he_ppe_th_size((u8 *)sta->he_capabilities);
		for (i = 0; i < he_ppe_size; i++) {
			ret = sprintf_s(buf + len, buflen - len, "%02x%s", *(he + i),
				i + 1 < he_ppe_size ? " " : "\n");
			if (ret <= 0)
				return len;
				      ...
				      const char *txtaddr)
{
  ...
  if (txtaddr)
	...
	if (pos) {
		reason = atoi(pos + 8);
		if (!IS_VALID_RANGE(reason, WLAN_REASON_UNSPECIFIED, WLAN_REASON_MESH_CHANNEL_SWITCH_UNSPECIFIED))
			return -1;
	}
			      ...
			      size_t buflen)
{
	...
	if (!iface->cac_started || !iface->dfs_cac_ms) {
		...
		if ((iface->dfs_cac_ms / 1000) > now.sec)
			left_time = iface->dfs_cac_ms / 1000 - now.sec;
		else
			/* CAC time may excceed planned CAC time in case CAC
			 * was paused */
			left_time = 0;
		ret = os_snprintf(buf + len, buflen - len,
				  "cac_time_seconds=%u\n"
	...
	for (i = 0; i < iface->num_bss; i++) {
		...
		if (os_snprintf_error(buflen - len, ret))
			return len;
		len += ret;
		if (NULL != bss->bkh_vrt_ifaces &&
		    bss->num_bkh_vrt_ifaces > 0) {
			int k, written = 0;

			for (k = 0; k < bss->num_bkh_vrt_ifaces; k++) {
				struct backhaul_virtual_iface *bkh_vrt_iface
							= bss->bkh_vrt_ifaces[k];
...
int hostapd_ctrl_iface_pmk_list(struct hostapd_data *hapd, char *buf,
				  size_t len)
{
