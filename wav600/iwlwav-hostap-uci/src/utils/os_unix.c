...
size_t os_strlcat(char *dest, const char *src, size_t siz)
{
	...
	size_t n = siz;
	size_t dlen;
	rsize_t slen = strnlen_s(src, RSIZE_MAX_STR);
	if (!IS_VALID_RANGE(slen, AP_MIN_VALID_CHAR, RSIZE_MAX_STR - 1)) {
#ifdef WPA_TRACE
		wpa_printf(MSG_ERROR, "os_strlcat - wrong src len");
#endif /* WPA_TRACE */
		return 0;
	}
	...
	*d = '\0';
	...
	return(dlen + (s - src));	/* does not include NUL */
}
