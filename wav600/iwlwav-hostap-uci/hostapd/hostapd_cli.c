					...
					char *argv[])
{
	...
	res = sprintf_s(cmd, sizeof(cmd), "SET_NEIGHBOR %s %s %s %s %s %s",
...
static int hostapd_cli_cmd_get_zwdfs_antenna(struct wpa_ctrl *ctrl, int argc,
					     char *argv[])
{
	char cmd[32];
	int res;
...
static int hostapd_cli_cmd_update_rnr(struct wpa_ctrl *ctrl,int argc, char *argv[])
{

	if (argc != 9) {
		printf("Invalid update_rnr command - requires <BSSID> <OperatingClass>"
			" <Channel> <SSID> <multibss_enable> <is_transmitted_bssid> <unsolicited_frame_support>"
			" <max_tx_power> <hiden_mode>\n");
		return -1;
	}

	return hostapd_cli_cmd(ctrl, "UPDATE_RNR", 9, argc, argv);
}

static int hostapd_cli_cmd_delete_rnr(struct wpa_ctrl *ctrl,int argc, char *argv[])
{
	char cmd[32];
	int res;

	if (argc != 1) {
		printf("Invalid delete_rnr command - requires <BSSID>\n");
		return -1;
	}

	res = os_snprintf(cmd, sizeof(cmd), "DELETE_RNR %s", argv[0]);
	if (os_snprintf_error(sizeof(cmd), res)) {
		printf("Too long delete_rnr command.\n");
		return -1;
	}
	return wpa_ctrl_command(ctrl, cmd);
}
...
static int hostapd_cli_cmd_get_unsolicited_frame_duration(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[32];
	int res = 0;

	res = sprintf_s(cmd, sizeof(cmd), "GET_UNSOLICITED_FRAME_DURATION");

	if (res <= 0) {
		printf("%s: get_unsolicited_frame_duration command, snprintf error %d\n", __func__, res);
		return -1;
	}
...
static int hostapd_cli_cmd_set_unsolicited_frame_duration(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[64];
	int res = 0;
	int total, i;
        ...
        if (argc != 1) {
                printf("Invalid SET_UNSOLICITED_FRAME_DURATION command\n"
                                "usage: <time_inms ( <= 20ms ) >\n");
                return -1;
        }

        res = sprintf_s(cmd, sizeof(cmd), "SET_UNSOLICITED_FRAME_DURATION");
        if (res <= 0)
                goto err;
        ...
        err: printf("Too long set_unsolicited_frame_duration command.\n");
        return -1;
}

static int hostapd_cli_cmd_get_unsolicited_frame_support(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[32];
	int res = 0;

	res = sprintf_s(cmd, sizeof(cmd), "GET_UNSOLICITED_FRAME_SUPPORT");

	if (res <= 0) {
		printf("%s: get_unsolicited_frame_support cmd, sprintf_s error %d\n", __func__, res);
		return -1;
	}
...
static int hostapd_cli_cmd_set_unsolicited_frame_support(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[64];
	int res = 0;
        int total, i;
        ...
        if (argc != 1) {
                printf("Invalid SET_UNSOLICITED_FRAME_SUPPORT command\n"
                                "usage: <0/1/2>\n");
                return -1;
        }

        res = sprintf_s(cmd, sizeof(cmd), "SET_UNSOLICITED_FRAME_SUPPORT");
        if (res <= 0)
            goto err;
        ...
        err: printf("Too long set_unsolicited_frame_support command.\n");
        return -1;
}
...
static int hostapd_cli_cmd_send_action_frame(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
...
static int hostapd_cli_cmd_get_sta_softblock(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	char cmd[256];
	int res;
	char *tmp;
...
static int hostapd_cli_cmd_acs_report_all_ch(struct wpa_ctrl *ctrl,
					     int argc, char *argv[])
{
	char cmd[25];
	int res;
		...
		char *argv[])
{
	...
	res = sprintf_s(cmd, sizeof(cmd), "MESH_MODE %s %s", argv[0], argv[1]);
	if (res <= 0) {
		printf("%s: sprintf_s error %d", __func__, res);
		return -1;
	}
		...
		char *argv[])
{
	...
	res = sprintf_s(cmd, sizeof(cmd), "GET_MESH_MODE %s", argv[0]);
	if (res <= 0) {
		printf("%s: sprintf_s error %d", __func__, res);
		return -1;
	}
...
static int hostapd_cli_cmd_reset_acs_state(struct wpa_ctrl *ctrl, int argc,
		char *argv[])
{
	char cmd[64];
	int res;
		...
		char *argv[], const char *cli_cmd_prefix, const char *ctlr_cmd_prefix)
{
	...
	if (argc == 1) {
		res = sprintf_s(cmd, sizeof(cmd), "%s_ELEMENTS %s", ctlr_cmd_prefix, argv[0]);
		if (res <= 0) {
			printf("%s: sprintf_s error %d", __func__, res);
			return -1;
		}
	} else {
		res = sprintf_s(cmd, sizeof(cmd), "%s_ELEMENTS %s %s", ctlr_cmd_prefix, argv[0], argv[1]);
		if (res <= 0) {
			printf("%s: sprintf_s error %d", __func__, res);
			return -1;
		}
		...
		char *argv[], const char *cli_cmd_prefix, const char *ctlr_cmd_prefix)
{
	...
	res = sprintf_s(cmd, sizeof(cmd), "GET_%s_ELEMENTS %s", ctlr_cmd_prefix, argv[0]);
	if (res <= 0) {
		printf("%s: sprintf_s error %d", __func__, res);
		return -1;
	}
...
static const struct hostapd_cli_cmd hostapd_cli_commands[] = {
	...
	{ "get_sta_softblock", hostapd_cli_cmd_get_sta_softblock, NULL,
	  "<bss> <addr>" },
	...
	{ "reset_acs_state", hostapd_cli_cmd_reset_acs_state, NULL,
	  " = reset acs state" },
	...
	{ "acs_report", hostapd_cli_cmd_acs_report, NULL,
	  "get ACS report" },
	{ "acs_report_all_ch", hostapd_cli_cmd_acs_report_all_ch, NULL,
	...
	{ "accept_acl", hostapd_cli_cmd_accept_macacl, NULL,
	  "<BSS_name> <ADD_MAC/DEL_MAC/SHOW/CLEAR> = accept MAC ACL" },
	{ "deny_acl", hostapd_cli_cmd_deny_macacl, NULL,
	  "<BSS_name> <ADD_MAC/DEL_MAC/SHOW/CLEAR/GET_PARAMS> [TIMER] = deny MAC ACL" },
	...
	{ "get_zwdfs_antenna", hostapd_cli_cmd_get_zwdfs_antenna, NULL,
	  " = get ZWDFS antenna Enabled/Disabled"},
	{ "update_rnr", hostapd_cli_cmd_update_rnr, NULL,
	  " = Include rnr report for 6ghz ap, <BSSID> <OperatingClass>"
	  " <Channel> <SSID> <multibss_enable> <is_transmitted_bssid> <unsolicited_frame_support>"
	  " <max_tx_power"},
	{ "delete_rnr", hostapd_cli_cmd_delete_rnr, NULL,
	  " = Exclude rnr report for 6ghz ap, <BSSID>"},
	...
	{ "get_unsolicited_frame_support", hostapd_cli_cmd_get_unsolicited_frame_support, NULL,
	  " = get unsolicited_frame_support value"},
	{ "set_unsolicited_frame_support", hostapd_cli_cmd_set_unsolicited_frame_support, NULL,
	  " = set unsolicited_frame_support value"},
	{ "get_unsolicited_frame_duration", hostapd_cli_cmd_get_unsolicited_frame_duration, NULL,
	  " = get unsolicited_frame_duration value"},
	{ "set_unsolicited_frame_duration", hostapd_cli_cmd_set_unsolicited_frame_duration, NULL,
	  " = set unsolicited_frame_duration value"},
