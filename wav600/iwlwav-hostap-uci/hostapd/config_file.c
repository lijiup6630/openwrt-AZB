		...
		struct hostapd_bss_config *bss, char *val)
{
	...
	if( (found == false) )
	{
		for (  j = MASTER_VAP_BSS_IDX + 1; ( j < MAX_SUPPORTED_6GHZ_VAP_PER_RADIO ) && (found == false) ; j++ ) 
		{
...
#define MAX_OPT_CH_LIST 5
	...
	*int_list = NULL;
	...
	list = os_malloc(sizeof(int) * (count+1));
	if (list == NULL)
		return -1;
	pos = val;
	count = 0;
	while (*pos != '\0') {
		...
		/* Skip PSC channels */
		if (is_6ghz_psc_freq(hostapd_chan_to_freq(atoi(pos), op_class))) {
			goto skip_psc;
		}
	...
	final_list = os_realloc(list, (sizeof(int) * count));
...
static int add_r0kh(struct hostapd_bss_config *bss, char *value)
{
	...
	if (next)
		*next++ = '\0';
	if (next == NULL || next - pos - 1 > FT_R0KH_ID_MAX_LEN) {
			       ...
			       const char *buf, char *pos, int line)
{
    ...
    if (bss->num_res_sta < 0) {
      ...
      return 1;
		...
		conf->request_credentials = (atoi(pos)) ? 1 : 0;
	...
	/* Smart ACS */
		...
		*/
		...
		if (hostapd_parse_opt_ch_list(&conf->acs_6g_opt_ch_list, pos, &conf->acs_6g_opt_ch_count, conf->op_class)) {
		...
		if (!IS_VALID_RANGE(val, 0, 30)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid acs_rnr_scan_timeout %d",
			line, val);
			return 1;
		}
		conf->acs_rnr_scan_timeout = val;
	}
#endif /* CONFIG_ACS */
	  ...
	  conf->vht_mcs_set[0] = host_to_le32(atoi(pos));
		...
		int val = atoi(pos);
		if (!IS_VALID_BOOL_RANGE(val)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid he_operation_cohosted_bss %d",
				line, val);
			return 1;
		}
		clr_set_he_cap(&conf->he_oper.he_oper_params[HE_OPERATION_CAP1_IDX],
					   val, HE_OPERATION_CAP1_CO_LOCATED_BSS);
		...
		conf->he_oper_er_su_disable_override = 1;
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(corrupt_gtk_rekey_mic_probability)
		...
		int val = atoi(pos);
		if (!IS_VALID_RANGE(val, RSSI_IGNORE_PROBE_REQ_LOWER, RSSI_IGNORE_PROBE_REQ_UPPER)) {
			wpa_printf(MSG_ERROR, "Line %d: invalid rssi_ignore_probe_request value %d",
				line, val);
			return 1;
		}
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
	...
	for (i = 0; i < conf->num_bss; i++) {
		conf->bss[i]->ssid.request_credentials = conf->request_credentials;
