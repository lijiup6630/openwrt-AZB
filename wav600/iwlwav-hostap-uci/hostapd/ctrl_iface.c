					  ...
					  char *pos)
{
	...
	bool iface_enabled = (iface->state == HAPD_IFACE_ENABLED);
	...
	if (settings.freq_params.freq == 0) {
		...
		hostapd_set_conf_from_freq_params(iface, &settings.freq_params);
	...
	if (iface->cac_started) {
		if (hostapd_disable_iface(iface) < 0) {
			wpa_printf(MSG_ERROR, "Channel switch: Disabling of interface failed");
			return -1;
		}

		if (hostapd_enable_iface(iface) < 0) {
			wpa_printf(MSG_ERROR, "Channel switch: Enabling of interface failed");
			return -1;
		}
		return 0;
	}

	/* check CAC required OR
	 * interface was not fully started yet - try to enable it with specified channel */
	if (!iface_enabled || ret) {
		if (ret && switch_type_scan)
			iface->block_tx = true;
                ...
                char *cmd)
{
	...
	{ /*TODO (6GHz) how to find new radio or existing? channel/op_class can be same, coloc as of now hardcoded so can't use for knowing if new radio
		...
		{
			for (  j = MASTER_VAP_BSS_IDX + 1; ( j < MAX_SUPPORTED_6GHZ_VAP_PER_RADIO ) && !found ;  ) 
			{
				char empty_bssid[ETH_ALEN] = {'\0'};
...
void hostapd_global_ctrl_iface_deinit(struct hapd_interfaces *interfaces)
{
	...
	/* Free RNR Info */
	ieee80211_rnr_info_delete(&interfaces->rnr_scan_6g_ap_info);
	...
	dl_list_for_each_safe(dst, next, ctrl_dst, struct wpa_ctrl_dst, list) {
		if ((level >= dst->debug_level) &&
			...
			if (sendmsg(sock, &msg, 0) < 0) {
				int _errno = errno;
				wpa_printf(MSG_DEBUG, "CTRL_IFACE monitor[%d]: "
					   "%d - %s",
