...
void PHY_Reset(uint32 band, uint32 value)
{
	...
	if (band == 2)
	{
		phy_ext_reset.bitFields.macToPhyBand2ResetN = value;
	}
        else
#endif 
	if (band == 1)
...
{
#ifdef ENET_INC_ARCH_WAVE700
	UNUSED_PARAM(ms_access);
	UNUSED_PARAM(rf_access);
	UNUSED_PARAM(addr);
	UNUSED_PARAM(rd_data);
...
void FCSI_WRITE_TRANSACTION_ANT(uint16 ms_access, uint16 rf_access, uint16 ant_mask, uint16 addr, uint16* data, bool* success)
{
	...
	if (*success == TRUE)
	{
		for (ant = 0; ant < MAX_NUM_OF_RX_ANTENNAS; ant++)
		{
			if ((ant_mask & (1 << ant)) != 0)
			{
...
void PHY_SET_MAC_ANT_CONFIG(uint8 band, uint8 antMask)
{
		...
		//TODO: WLANRTSYS-32204: understand how to manage antennas in WAVE700
		if (band == 0)
		{
...
#if defined(RFIC_WRX600D2) && !defined(RFIC_TYPE_WRX700) && !defined(RFIC_TYPE_WRX700MIXED)
