  ...
  if (mtlk_mmb_dcdp_path_available(hw)) {
    ...
    if (mtlk_hw_type_is_gen6_d2(hw)) {
      ...
      if (wave_radio_get_is_zwdfs_radio(radio)) {
        ELOG_S("%s: ZWDFS radio - Can't register DirectConnect subinterface",
                                              ...
                                              const mtlk_coc_power_cfg_t *power_params)
{
  ...
  intvec[9] = power_params->bw_params.coc_BW;
  ILOG3_D("mask 0x%08X", power_params->ant_mask_params.word);

  MTLK_STATIC_ASSERT(10 == MTLK_COC_PW_MAX_CFG_PARAMS);
...
{
  static const char* modes[] = { "802.11a/g", "802.11b", "802.11n", "802.11ac", "802.11ax" , "802.11ax_su_ext", "802.11ax_mu_trig", "11ac/mu", "11ax/bf","11ac/bf"};
...
#undef _IS_QAM256_OR_HIGHER
	...
	if ((MTLK_HW_BAND_6_GHZ == tx_pw_data->cur_band) && (!(is_11ax || entry->phy_mode == PHY_MODE_11AX_BF_DL))) continue;
    ...
    for (i = 0; i < pw_size; i++) {
      ...
      if (is_11b) {
		...
		power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[PSDB_PHY_CW_OFDM_20 + i]);
      }
	  else if(entry->phy_mode == PHY_MODE_11AX_MU_DL){
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[PSDB_PHY_CW_MU_20 + i];
		power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[PSDB_PHY_CW_MU_20 + i]);
	  }
	  else if(entry->phy_mode == PHY_MODE_11AC_BF_DL){
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[PSDB_PHY_CW_BF_20 + i];
		power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[PSDB_PHY_CW_BF_20 + i]);
	  }
	  else if(entry->phy_mode == PHY_MODE_11AX_BF_DL){
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[PSDB_PHY_CW_BF_AX_20 + i];
		power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[PSDB_PHY_CW_BF_AX_20 + i]);
	  }
      else if (is_11n){
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[PSDB_PHY_CW_N_20 + i];
        power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[PSDB_PHY_CW_N_20 + i]);
      }
      else if (is_11ag){
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[PSDB_PHY_CW_AG_20];
        power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[PSDB_PHY_CW_AG_20]);
      } else {
        power_cfg = tx_pw_data->power_tpc_cfg.pw_limits[i + 1];
        power_cfg = hw_mmb_get_tx_power_target(power_cfg, tx_pw_data->power_tpc_psd.pw_limits[i + 1]);
      }
...
#if defined(MTLK_LGM_PLATFORM) || defined(MTLK_PUMA_PLATFORM)
{
      wave_radio_t *radio = wave_vap_radio_get(vap_handle);
      if (wave_radio_is_5g_in_zwdfs_cdb_cfg(radio)) {
        dc_dp_subif.vap_id = mtlk_vap_get_id_fw(vap_handle);
      }
}
#endif
      dc_dp_subif.port_id = port_id;
...
_mtlk_df_dcdp_get_peripheral_config (mtlk_hw_t *hw, mtlk_dcdp_dev_t *dp_dev, BOOL zwdfs)
{
...
#else
  {
    ...
    if (wave_hw_radio_band_cfg_is_single(hw)) {
      ...
      if (mtlk_hw_type_is_gen7(hw) && !wave_hw_type_is_asic(hw))
...
/* Register and initialize datapath device */
{
  ...
  if (MTLK_DC_DP_MODE_UNREGISTERED == dp_dev->dp_mode)
  {
    ...
    if (mtlk_hw_type_is_gen7(hw)) {
      extra_flag[0] = (DC_DP_F_FAST_WLAN_2_4G | DC_DP_F_FAST_WLAN_EXT);
      extra_flag[1] |= (DC_DP_F_FAST_WLAN_5G | DC_DP_F_FAST_WLAN_EXT);
      extra_flag[2] |= (DC_DP_F_FAST_WLAN_6G | DC_DP_F_FAST_WLAN_EXT);
    ...
    if (DC_DP_FAILURE != dp_dev->dp_port_id[0]) {
      ...
      if ((flags & DC_DP_F_FASTPATH) && (num_radios > 1) && (!zwdfs)) {
      ...
      for (i = 0; i < num_radios; i++) {
        ...
        dp_dev->dp_port_id[i] = dc_dp_alloc_port(THIS_MODULE, dev_port, dp_dev->dp_ndev[i], port_id, flags | extra_flag[i] | DC_DP_F_FAST_WLAN);
