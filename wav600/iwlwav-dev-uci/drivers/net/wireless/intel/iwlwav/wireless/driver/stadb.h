...
typedef struct wave_mhi_la_ofdma_mu_ru_stats {
  /* Packet error ru percent */
  uint32 pkt_err_ru_percent;
  uint32 ru_percent;
  uint64 success_ru_ampdus;
  uint64 failed_ru_ampdus;
  uint64 total_tx_ru_ampdus;
} wave_mhi_la_ofdma_mu_ru_stats_t;

typedef struct wave_mhi_la_ofdma_su_mu_ru_stats {
  /* Packet error su percent */
  uint32 pkt_err_su_percent;
  /* Packet error mu percent */
  uint32 pkt_err_mu_percent;
  /* total_su_ampdus_percent  = total_su_ampdus *100 / total_tx_ampdus */
  uint32 total_su_ampdus_percent;
  /* total_mu_ampdus_percent  = total_mu_ampdus *100 / total_tx_ampdus */
  uint32 total_mu_ampdus_percent;
  uint32 total_pkt_err_percent;
  uint64 success_su_ampdus;
  uint64 failed_su_ampdus;
  uint64 total_su_ampdus;
  uint64 success_mu_ampdus;
  uint64 failed_mu_ampdus;
  uint64 total_mu_ampdus;
  uint64 total_tx_ampdus;
  uint64 total_fail_ampdus;
  /* total_tx_mu_ampdus = Sum of the total_tx_ru_ampdus of all Ru ids */
  uint64 total_tx_mu_ampdus;
  wave_mhi_la_ofdma_mu_ru_stats_t sta_mu_ru[NUM_OF_RU_SIZES];
} wave_mhi_la_ofdma_su_mu_ru_stats_t;
...
typedef struct mtlk_mhi_sta_stats{
    ...
    uint32 staRemovedFavoringSuCounter;
    uint32 staNotAddedNbiqBelowThresholdCounter;
    uint32 success_su_ampdus;
    uint32 failed_su_ampdus;
    uint32 success_ru_ampdus[NUM_OF_RU_SIZES];
    uint32 failed_ru_ampdus[NUM_OF_RU_SIZES];
...
typedef struct mtlk_mhi_sta_stats64{
    ...
    uint64 success_su_ampdus;
    uint64 failed_su_ampdus;
    uint64 success_ru_ampdus[NUM_OF_RU_SIZES];
    uint64 failed_ru_ampdus[NUM_OF_RU_SIZES];
...
typedef struct _sta_stats
{
  ...
  wave_mhi_la_ofdma_su_mu_ru_stats_t su_mu_ru_stats;
...
static __INLINE wave_mhi_la_ofdma_su_mu_ru_stats_t *
wave_sta_get_ofdma_su_mu_ru_stats (const sta_entry *sta)
{
  return &((sta_entry *)sta)->info.stats.su_mu_ru_stats;
}
