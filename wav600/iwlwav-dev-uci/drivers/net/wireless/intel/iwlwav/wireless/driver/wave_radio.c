...
((wave_hw_radio_band_cfg_get(_radio->hw_api->hw) == WAVE_HW_RADIO_BAND_CFG_SCAN) && \
 (wave_radio_band_get(_radio) == MTLK_HW_BAND_5_2_GHZ))
...
static int _wave_radio_master_vap_create(wave_radio_t *radio, mtlk_work_role_e role)
{
  ...
  if (wave_radio_get_is_zwdfs_radio(radio)) {
    wlan_id = MTLK_MAX_HW_ADAPTERS_SUPPORTED - 1; /* get zwdfs interface index (wlan6) */
  } else {
...
mtlk_error_t wave_radio_get_MuOFDMA_stats (
  struct wiphy *wiphy,
  struct net_device *ndev)
{
  mtlk_df_user_t *df_user;
  ...
  mtlk_error_t res = MTLK_ERR_OK;
  uint32 size, *no_of_stas, sta_cnt;
  wave_wssa_link_adapt_mu_ofdma_stats_t *lamuofdma_stats = NULL, *tmp_stats = NULL, *tmp_clpb = NULL;
  ...
  res = _mtlk_df_user_invoke_core(mtlk_df_user_get_df(df_user),
    WAVE_CORE_REQ_GET_LAMUOFDMA_STATS, &clpb, NULL, 0);
  res = _mtlk_df_user_process_core_retval_void(res, clpb,
    WAVE_CORE_REQ_GET_LAMUOFDMA_STATS, FALSE);
  ...
  if (0 != *no_of_stas) {
    if (NULL == (lamuofdma_stats = mtlk_osal_mem_alloc(sizeof(wave_wssa_link_adapt_mu_ofdma_stats_t) * (*no_of_stas),
                                                MTLK_MEM_TAG_EXTENSION))) {
      ELOG_V("Can't allocate memory");
      res = MTLK_ERR_NO_MEM;
      goto end;
    ...
    while ((NULL != (tmp_clpb = mtlk_clpb_enum_get_next(clpb, &size))) && ((*no_of_stas)--)) {
      ...
      wave_memcpy(tmp_stats++, size, tmp_clpb, size);
    }

    res = wv_cfg80211_vendor_cmd_alloc_and_reply(wiphy, lamuofdma_stats, sizeof(*lamuofdma_stats) * sta_cnt);
    mtlk_clpb_delete(clpb);
    mtlk_osal_mem_free(lamuofdma_stats);
    return res;
  }
  else
  {
                                           ...
                                           wave_vendor_su_mu_ru_stats_t *data)
{
 ...
 if ((data->category != OFDMA_CATEGORY_TOTAL_STATS) && (!(mtlk_osal_is_valid_ether_addr(data->sta_addr.au8Addr)))) {
  ...
  if (data->category == OFDMA_CATEGORY_MU_RU)
    payload_size = sizeof(wave_wssa_ofdma_su_mu_ru_stats_t);
  else if (data->category == OFDMA_CATEGORY_SU_MU)
    payload_size = sizeof(wave_wssa_ofdma_su_mu_stats_t);
  else if (data->category == OFDMA_CATEGORY_TOTAL_STATS)
