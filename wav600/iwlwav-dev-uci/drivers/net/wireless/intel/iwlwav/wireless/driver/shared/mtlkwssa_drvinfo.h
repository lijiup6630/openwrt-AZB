...
/* OFDMA Su_Mu_Ru stats */
/* TODO: Handling of WAVE700_NUM_OF_RU_SIZES should be taken care, When the support for WAVE700 */
/* boards added independently. Since the sizeof(StatisticsDb_t_wave600d2) == sizeof(StatisticsDb_t_wave700) */
/* in the present implemenation, NUM_OF_RU_SIZES will be same for both the WAVE700 and WAVE600D2 boards. */
#define NUM_OF_RU_SIZES                  WAVE600_NUM_OF_RU_SIZES
#define OFDMA_CATEGORY_MU_RU             (0)
#define OFDMA_CATEGORY_SU_MU             (1)
#define OFDMA_CATEGORY_TOTAL_STATS       (0x0F)
...
typedef struct wave_wssa_drv_link_adapt_mu_ofdma_stats {
  uint16 sta_sid;
  IEEE_ADDR addr;
  uint32 staRemovedNbiqBelowThresholdCounter;
  uint32 staRemovedFavoringSuCounter;
  uint32 staNotAddedNbiqBelowThresholdCounter;
} wave_wssa_link_adapt_mu_ofdma_stats_t;

typedef struct wave_wssa_ofdma_mu_ru_stats {
  uint32 mu_ru_pkt_err_ru_percent;
  uint32 mu_ru_ru_percent;
  uint64 mu_ru_success_ru_ampdus;
  uint64 mu_ru_failed_ru_ampdus;
  uint64 mu_ru_total_tx_ru_ampdus;
} wave_wssa_ofdma_mu_ru_stats_t;

typedef struct wave_wssa_ofdma_su_mu_ru_stats {
  uint16 sta_sid;
  IEEE_ADDR addr;
  uint64 total_tx_mu_ampdus;
  wave_wssa_ofdma_mu_ru_stats_t mu_ru_stats[NUM_OF_RU_SIZES];
} wave_wssa_ofdma_su_mu_ru_stats_t;

typedef struct wave_wssa_ofdma_total_stats {
  uint64 all_sta_success_su_ampdus;
  uint64 all_sta_failed_su_ampdus;
  uint64 all_sta_success_ru_ampdus[NUM_OF_RU_SIZES];
  uint64 all_sta_failed_ru_ampdus[NUM_OF_RU_SIZES];
} wave_wssa_ofdma_total_stats_t;

typedef struct wave_wssa_ofdma_su_mu_stats {
  uint16 sta_sid;
  IEEE_ADDR addr;
  uint32 su_pkt_err_percent;
  uint32 mu_pkt_err_percent;
  uint32 total_su_pkts_percent;
  uint32 total_mu_pkts_percent;
  uint32 total_pkt_err_percent;
  uint64 success_su_pkts;
  uint64 failed_su_pkts;
  uint64 success_mu_pkts;
  uint64 failed_mu_pkts;
  uint64 total_tx_pkts;
  uint64 total_fail_pkts;
} wave_wssa_ofdma_su_mu_stats_t;
