...
_mtlk_sta_get_peer_traffic_stats (const sta_entry* sta, mtlk_wssa_peer_traffic_stats_t* stats)
{
  ...
  stats->PacketsSent      = (stat_cntrs->mpduTransmitted - stat_cntrs_snapshot->mpduTransmitted);
  stats->PacketsReceived  = (stat_cntrs->rdCount - stat_cntrs_snapshot->rdCount);
  stats->BytesSent        = (stat_cntrs->mpduByteTransmitted - stat_cntrs_snapshot->mpduByteTransmitted);
  stats->BytesReceived    = (stat_cntrs->rxOutStaNumOfBytes - stat_cntrs_snapshot->rxOutStaNumOfBytes);
