...
((_chipid_is_gen6_a0(id)) ? MTLK_DIFF_STATS_A0(stats_dst, stats_src, parameter) : \
((_chipid_is_gen6_b0(id)) ? MTLK_DIFF_STATS_B0(stats_dst, stats_src, parameter) : \
MTLK_DIFF_STATS_D2(stats_dst, stats_src, parameter)))

#define MTLK_DIFF_STATS_A0(stats_dst, stats_src, parameter) \
(((StatisticsDb_t_wave600*)stats_dst)->parameter - ((StatisticsDb_t_wave600*)stats_src)->parameter )
...
#if WAVE_USE_BSS_TX_MONITOR
#ifdef MTLK_WAVE_700
  bss_tx_hd_map_t       bss_tx_hd_map[MGMT_TX_RING_SIZE];
#else
...
static __INLINE unsigned compute_stat_index(mtlk_hw_t *hw, int channel_num)
{
  mtlk_hw_band_e band = wave_radio_band_get(wave_hw_main_radio_get(hw));
  return((MTLK_HW_BAND_6_GHZ == band)? channum6g2cssidx(channel_num) : channum2cssidx(channel_num));
}
                      ...
                      unsigned long    size)
{
  ...
  if (_hw_type_is_gen7(hw)) {
    MTLK_ASSERT(fwnum <= WAVE_FW_NUM_IF);
  }
#endif
...
wave_get_core_from_vap_id (mtlk_core_t *master_nic)
{
  mtlk_hw_t *hw = mtlk_vap_get_hw(master_nic->vap_handle);
  uint8 radio_id = wave_vap_radio_id_get(master_nic->vap_handle);
  mtlk_vap_handle_t vap_handle;

  vap_handle = _mtlk_mmb_get_vap_handle_from_vap_id(hw, radio_id, master_nic->csi_vap_id);
...
wave_rtlog_packet_is_csi(struct sk_buff *skb, uint32 *udp_data_len)
{
  ...
  *udp_data_len = csi_rtlog_pck_hdr->udph.len;
  ...
  if (ntohs(csi_rtlog_pck_hdr->udph.dest) == WAVE_RTLOG_CSI_UDP_DEST_PORT &&
    ...
    mtlk_dump(2, skb->data, 80, "CSI data recv from Phy");
    return TRUE;
  }
...
#if defined(MTLK_LGM_PLATFORM) || defined(MTLK_PUMA_PLATFORM)
{ /* In LGM, DCDP does not support CDB, so there is no translation of VAPID involved in DCDP */
  /* So, Convert absolute vapid to relative vapid */
  wave_radio_t *radio = wave_vap_radio_get(ndev_vap_handle);
  if (wave_radio_is_5g_in_zwdfs_cdb_cfg(radio)) {
    uint8 max_vaps_count = mtlk_vap_manager_get_max_vaps_count(mtlk_vap_get_manager(ndev_vap_handle));
    vap_id = dc_dp_rx_subif.vap_id & (max_vaps_count - 1);
  }
}
#endif
...
BOOL __MTLK_IFUNC
wave_hw_type_is_asic (mtlk_hw_t *hw)
{
  MTLK_ASSERT(hw);
  return __hw_mmb_card_is_asic(hw);
}
...
mtlk_hw_mhi_get_vap_stats (mtlk_hw_t *hw, mtlk_mhi_stats_vap_t *vap_stats, unsigned vap_id)
{
  ...
  if (_chipid_is_gen6_a0(chip_id)) {
    stats->amsdu                              = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.vapCounts[vap_id].amsdu);
    stats->amsduBytes                         = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.vapCounts[vap_id].amsduBytes);
    stats->dropCount                          = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.vapCounts[vap_id].dropCount);
    stats->mpduInAmpdu                        = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.vapCounts[vap_id].mpduInAmpdu);
    stats->octetsInAmpdu                      = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.vapCounts[vap_id].octetsInAmpdu);
    stats->rxClassifierSecurityMismatch       = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.vapCounts[vap_id].rxCoordinatorSecurityMismatch);
    stats->tkipCount                          = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.rxppVapCounts[vap_id].tkipCount);
    stats->securityFailure                    = MTLK_DIFF_STATS_A0(hw_stats->stats_copy, hw_stats->stats_snapshot, rxCounters.rxppVapCounts[vap_id].securityFailure);
...
static void
_wave_sta_link_adapt_ofdma_su_mu_ru_stats (sta_entry *sta)
{
    wave_mhi_la_ofdma_su_mu_ru_stats_t *sta_su_mu_ru_stats;
    mtlk_mhi_stats64_sta_cntr_t *stats64;
    mtlk_mhi_stats64_sta_cntr_t *stats64_snapshot;
    uint32              ru_id;
    uint64              success_ru_ampdus = 0, failed_ru_ampdus = 0;
    uint64              total_success_ru_ampdus = 0, total_failed_ru_ampdus = 0, tx_mu_ampdus = 0;

    if (sta == NULL) {
      return;
    }

    sta_su_mu_ru_stats = wave_sta_get_ofdma_su_mu_ru_stats(sta);
    stats64 = mtlk_sta_get_mhi_stat64_array(sta);
    stats64_snapshot = mtlk_sta_get_mhi_stat64_snapshot_array(sta);

    sta_su_mu_ru_stats->success_su_ampdus = stats64->success_su_ampdus - stats64_snapshot->success_su_ampdus;
    sta_su_mu_ru_stats->failed_su_ampdus = stats64->failed_su_ampdus - stats64_snapshot->failed_su_ampdus;

    for (ru_id = 0; ru_id < NUM_OF_RU_SIZES; ru_id++) {
       success_ru_ampdus = stats64->success_ru_ampdus[ru_id] - stats64_snapshot->success_ru_ampdus[ru_id];
       failed_ru_ampdus  = stats64->failed_ru_ampdus[ru_id] - stats64_snapshot->failed_ru_ampdus[ru_id];
       total_success_ru_ampdus += success_ru_ampdus;
       sta_su_mu_ru_stats->sta_mu_ru[ru_id].success_ru_ampdus = success_ru_ampdus;

       total_failed_ru_ampdus += failed_ru_ampdus;
       sta_su_mu_ru_stats->sta_mu_ru[ru_id].failed_ru_ampdus = failed_ru_ampdus;
       /* Parameters for each RU on each STA */
       sta_su_mu_ru_stats->sta_mu_ru[ru_id].total_tx_ru_ampdus  = sta_su_mu_ru_stats->sta_mu_ru[ru_id].success_ru_ampdus +
                                                                  sta_su_mu_ru_stats->sta_mu_ru[ru_id].failed_ru_ampdus;

       tx_mu_ampdus  += sta_su_mu_ru_stats->sta_mu_ru[ru_id].total_tx_ru_ampdus;
    }
    /* Parameters for each STA */
    sta_su_mu_ru_stats->total_su_ampdus         = sta_su_mu_ru_stats->success_su_ampdus + sta_su_mu_ru_stats->failed_su_ampdus;
    sta_su_mu_ru_stats->success_mu_ampdus       = total_success_ru_ampdus;
    sta_su_mu_ru_stats->failed_mu_ampdus        = total_failed_ru_ampdus;
    sta_su_mu_ru_stats->total_mu_ampdus         = sta_su_mu_ru_stats->success_mu_ampdus + sta_su_mu_ru_stats->failed_mu_ampdus;
    sta_su_mu_ru_stats->total_tx_ampdus         = sta_su_mu_ru_stats->total_su_ampdus + sta_su_mu_ru_stats->total_mu_ampdus;
    sta_su_mu_ru_stats->total_tx_mu_ampdus      = tx_mu_ampdus;
    sta_su_mu_ru_stats->total_fail_ampdus       = sta_su_mu_ru_stats->failed_su_ampdus + sta_su_mu_ru_stats->failed_mu_ampdus;
}
...
_wave_sta_link_adapt_ofdma_stats(mtlk_hw_t *hw, sta_entry *sta)
{
    ...
    _wave_sta_link_adapt_ofdma_su_mu_ru_stats(sta);
}
...
_mtlk_sta_update_statistics (mtlk_hw_t *hw, sta_entry *sta, stationPhyRxStatusDb_t *sta_status)
{
  ...
  int chip_id = hw_mmb_get_chip_id(hw);
  int agreement, buffer_status, ru_id;
  uint64 prev_stats_ampdus = 0;
  mtlk_core_t *cur_nic;
  wave_vap_su_mu_ru_stats_t *mhi_vap_ofdma_stats;
  ...
  cur_nic = mtlk_vap_get_core(sta->vap_handle);
  mhi_vap_ofdma_stats = &cur_nic->ofdma_stats;
  ...
  if (_chipid_is_gen6_b0(chip_id)) {
    ...
    __stats_accumulate_64bit(&stats64->staRemovedNbiqBelowThresholdCounter, &values->staRemovedNbiqBelowThresholdCounter,
       (MTLK_GET_STATS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[sta_sid].staRemovedNbiqBelowThresholdCounter)));
    __stats_accumulate_64bit(&stats64->staRemovedFavoringSuCounter, &values->staRemovedFavoringSuCounter,
       (MTLK_GET_STATS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[sta_sid].staRemovedFavoringSuCounter)));
    __stats_accumulate_64bit(&stats64->staNotAddedNbiqBelowThresholdCounter, &values->staNotAddedNbiqBelowThresholdCounter,
       (MTLK_GET_STATS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[sta_sid].staNotAddedNbiqBelowThresholdCounter)));
    /* su_mu_ru ofdma stats */
    prev_stats_ampdus = stats64->success_su_ampdus;
    __stats_accumulate_64bit(&stats64->success_su_ampdus, &values->success_su_ampdus,
       (MTLK_GET_STATS_B0(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].successSuAmpdus)));
    mhi_vap_ofdma_stats->vap_success_su_ampdus += (stats64->success_su_ampdus - prev_stats_ampdus);

    prev_stats_ampdus = stats64->failed_su_ampdus;
    __stats_accumulate_64bit(&stats64->failed_su_ampdus, &values->failed_su_ampdus,
       (MTLK_GET_STATS_B0(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].failedSuAmpdus)));
    mhi_vap_ofdma_stats->vap_failed_su_ampdus += stats64->failed_su_ampdus - prev_stats_ampdus;

    for (ru_id = 0; ru_id < NUM_OF_RU_SIZES; ru_id++) {
       prev_stats_ampdus = stats64->success_ru_ampdus[ru_id];
       __stats_accumulate_64bit(&stats64->success_ru_ampdus[ru_id], &values->success_ru_ampdus[ru_id],
          (MTLK_GET_STATS_B0(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].muRu[ru_id].successRuAmpdus)));
       mhi_vap_ofdma_stats->vap_success_ru_ampdus[ru_id] += stats64->success_ru_ampdus[ru_id] - prev_stats_ampdus;

       prev_stats_ampdus = stats64->failed_ru_ampdus[ru_id];
       __stats_accumulate_64bit(&stats64->failed_ru_ampdus[ru_id], &values->failed_ru_ampdus[ru_id],
          (MTLK_GET_STATS_B0(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].muRu[ru_id].failedRuAmpdus)));
       mhi_vap_ofdma_stats->vap_failed_ru_ampdus[ru_id] += stats64->failed_ru_ampdus[ru_id] - prev_stats_ampdus;
    }
    ...
    __stats_accumulate_64bit(&stats64->staRemovedNbiqBelowThresholdCounter, &values->staRemovedNbiqBelowThresholdCounter,
       (MTLK_GET_STATS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[sta_sid].staRemovedNbiqBelowThresholdCounter)));
    __stats_accumulate_64bit(&stats64->staRemovedFavoringSuCounter, &values->staRemovedFavoringSuCounter,
       (MTLK_GET_STATS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[sta_sid].staRemovedFavoringSuCounter)));
    __stats_accumulate_64bit(&stats64->staNotAddedNbiqBelowThresholdCounter, &values->staNotAddedNbiqBelowThresholdCounter,
       (MTLK_GET_STATS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[sta_sid].staNotAddedNbiqBelowThresholdCounter)));

    /* su_mu_ru ofdma stats */
    prev_stats_ampdus = stats64->success_su_ampdus;
    __stats_accumulate_64bit(&stats64->success_su_ampdus, &values->success_su_ampdus,
       (MTLK_GET_STATS_D2(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].successSuAmpdus)));
    mhi_vap_ofdma_stats->vap_success_su_ampdus += stats64->success_su_ampdus - prev_stats_ampdus;

    prev_stats_ampdus = stats64->failed_su_ampdus;
    __stats_accumulate_64bit(&stats64->failed_su_ampdus, &values->failed_su_ampdus,
       (MTLK_GET_STATS_D2(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].failedSuAmpdus)));
    mhi_vap_ofdma_stats->vap_failed_su_ampdus += stats64->failed_su_ampdus - prev_stats_ampdus;

    for (ru_id = 0; ru_id < NUM_OF_RU_SIZES; ru_id++) {
       prev_stats_ampdus = stats64->success_ru_ampdus[ru_id];
       __stats_accumulate_64bit(&stats64->success_ru_ampdus[ru_id], &values->success_ru_ampdus[ru_id],
          (MTLK_GET_STATS_D2(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].muRu[ru_id].successRuAmpdus)));
       mhi_vap_ofdma_stats->vap_success_ru_ampdus[ru_id] += stats64->success_ru_ampdus[ru_id] - prev_stats_ampdus;

       prev_stats_ampdus = stats64->failed_ru_ampdus[ru_id];
       __stats_accumulate_64bit(&stats64->failed_ru_ampdus[ru_id], &values->failed_ru_ampdus[ru_id],
          (MTLK_GET_STATS_D2(hw_stats->stats_copy, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[sta_sid].muRu[ru_id].failedRuAmpdus)));
       mhi_vap_ofdma_stats->vap_failed_ru_ampdus[ru_id] += stats64->failed_ru_ampdus[ru_id] - prev_stats_ampdus;
    }
  }
...
_wave_update_mhi_vap_PDStats (mtlk_hw_t *hw, sta_entry *sta)
{
  ...
  master_core = mtlk_vap_manager_get_master_core(mtlk_vap_get_manager(sta->vap_handle));
...
_mtlk_hw_check_and_convert_endianess (mtlk_hw_t *hw)
{
  ...
  for (i = 0; i < HW_NUM_OF_STATIONS; i++) {
    ...
    if (_chipid_is_gen6_b0(chip_id)) {
      ...
      MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[i].staRemovedNbiqBelowThresholdCounter);
      MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[i].staRemovedFavoringSuCounter);
      MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[i].staNotAddedNbiqBelowThresholdCounter);
      MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].successSuAmpdus);
      MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].failedSuAmpdus);
      for (ru_id = 0; ru_id < NUM_OF_RU_SIZES; ru_id++) {
         MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].muRu[ru_id].successRuAmpdus);
         MTLK_CONVERT_ENDIANESS_B0(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].muRu[ru_id].failedRuAmpdus);
      }
    } else if (_chipid_is_gen6_d2_or_gen7(chip_id)) {
      ...
      MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[i].staRemovedNbiqBelowThresholdCounter);
      MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[i].staRemovedFavoringSuCounter);
      MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.staCounts[i].staNotAddedNbiqBelowThresholdCounter);
      MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].successSuAmpdus);
      MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].failedSuAmpdus);
      for (ru_id = 0; ru_id < NUM_OF_RU_SIZES; ru_id++) {
         MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].muRu[ru_id].successRuAmpdus);
         MTLK_CONVERT_ENDIANESS_D2(hw_stats->stats_data, linkAdaptationMuOfdmaStats.suMuCounts.suMuStations[i].muRu[ru_id].failedRuAmpdus);
      }
    }
  }
  ...
  if (_chipid_is_gen6_b0(chip_id)) {
    MTLK_CONVERT_ENDIANESS_ARRAY_32_B0(hw_stats->stats_data, perClientStats.successByteCount);
  }
  else if (_chipid_is_gen6_d2_or_gen7(chip_id)) {
    MTLK_CONVERT_ENDIANESS_ARRAY_32_D2(hw_stats->stats_data, perClientStats.successByteCount);
  }
...
wave_core_reset_vap_stats(mtlk_core_t *core)
{
  ...
  for (vap_id = 0; vap_id < vaps_count; vap_id++) {
     if (mtlk_vap_manager_get_vap_handle(vap_mgr, vap_id, &vap_handle) == MTLK_ERR_OK) {
       ...
       /* Reset vap OFDMA stats */
       memset(&cur_nic->ofdma_stats, 0, sizeof(wave_vap_su_mu_ru_stats_t));
     }
  }
}
       ...
       ((FRAME_TYPE_ETHERNET == frame_type) && type == (IEEE80211_FTYPE_DATA >> 2) && hw->bss_mgmt.rx_eapol.data_size))
  {
    ...
    if ((subtype == MAN_TYPE_PROBE_RES) || (subtype == MAN_TYPE_BEACON))
    {
      ...
      if (radio) {
        ...
        idx = compute_stat_index(hw, channel_num);
...
#ifdef MTLK_WAVE_700
#define WAVE_HW_RX_HD_VAP_MASK_GEN7_64         0x3F
...
static wave_radio_limits_t hw_radio_limits_tb[] = {
  /* max_vaps | max_stas | master_vap_id */
  { 16, 127, 15 },  /* Current limitation for Gen7 */
  { 16, 127, 15 },
  { 32, 255, 31 }
};
#endif
...
static int _wave_hw_radio_band_cfg_init (mtlk_hw_t *hw)
{
  ...
  case WAVE_HW_RADIO_BAND_CFG_CTB:
    hw->rx_hd_vap_mask = WAVE_HW_RX_HD_VAP_MASK_GEN7_64;
...
/*  For WAVE700 SB will be FPGA/EMUL 2.4G */
  ...
  case WAVE_HW_RADIO_BAND_CFG_SCAN:
#ifdef MTLK_WAVE_700
    MTLK_ASSERT(!_hw_type_is_gen7(hw));
#endif
  ...
  case WAVE_HW_RADIO_BAND_CFG_CTB:
    ...
    if(radio_id == WAVE_HW_GEN7_6G_IDX) {
      ep_bit = WAVE_HW_RX_HD_EP_MASK_6G_GEN7;
    } else  {
      ep_bit = (radio_id ^ 1) & 1;
    }
	break;
...
void scan_get_aocs_info (mtlk_core_t *core, struct intel_vendor_channel_data *ch_data)
{
  ...
  idx = compute_stat_index(hw, ch_data->channel);
  if (idx != CHAN_IDX_ILLEGAL) {
...
static void _mtlk_hw_reset_chan_statistics (mtlk_hw_t *hw, unsigned idx, uint8 radio_id)
{
  ...
  hw->chan_statistics[idx].ch_noise                 = MIN_NOISE;
  hw->chan_statistics[idx].ch_non_80211_noise       = MIN_NOISE;
...
mtlk_hw_reset_chan_statistics (mtlk_hw_t *hw, int ch_num, uint8 radio_id)
{
  ...
  MTLK_ASSERT(NULL != hw);
  idx = compute_stat_index(hw, ch_num);
...
mtlk_hw_save_chan_statistics_info (mtlk_hw_t *hw, struct mtlk_chan_def *ccd, struct intel_vendor_channel_data *ch_data, BOOL ch_radar_noise, uint8 radio_id)
{
  ...
  if (idx != CHAN_IDX_ILLEGAL) {
    ...
    hw->chan_statistics[idx].ch_noise                  = ch_data->cwi_noise;
    ...
    /* Only chUtilizationBusyRx is not reset in FW, thus should be taken as is and not accumulated by Driver */
...
#define BSS_TX_AGING_MAX_ASIC  10000u /* ASIC and FPGA */
...
wave_hw_bss_tx_map_check(mtlk_hw_t *hw)
{
  ...
  for (i = 0; i < MTLK_ARRAY_SIZE(hw->bss_tx_hd_map); i++) {
	...
	if (sta) {
      /* Check if HD is stucked only if STA is active.
	     Incase of inactive STA - STA will be anyway disconnected after Inactivity timeout
		 This is to avoid False HD Stuck warning for inactive STA's */
      if (mtlk_sta_get_inactive_time(sta) < STA_INACTIVITY_THRES) {
        if (age > timeout) {
          ELOG_DDDDDYDDDD("HD stuck on card %d bd_idx %d radio %d vap %d sid %d mac %Y type %d stype %d age %d inactive time %u in ms",
...
wave_hw_cca_stats_get (mtlk_hw_t *hw, int channel, wave_cca_stats_t *cca_stats)
{
  ...
  hw_stats = &hw->hw_stats;
  idx = compute_stat_index(hw, channel);
...
wave_core_get_cur_channel_stats (mtlk_core_t *core, mtlk_wssa_drv_cur_channel_stats_t *stats)
{
  ...
  mtlk_osal_lock_acquire(&hw_stats->lock);
  stats->chUtilizationTotal = MTLK_DIFF_STATS(hw_stats->stats_data, hw_stats->stats_snapshot,  currentChannelStats.channelUtilizationStats[radio_idx].chUtilizationTotal, chip_id);
  stats->chUtilizationBusy = MTLK_DIFF_STATS(hw_stats->stats_data, hw_stats->stats_snapshot,  currentChannelStats.channelUtilizationStats[radio_idx].chUtilizationBusy, chip_id);
  stats->chUtilizationBusyTx = MTLK_DIFF_STATS(hw_stats->stats_data, hw_stats->stats_snapshot,  currentChannelStats.channelUtilizationStats[radio_idx].chUtilizationBusyTx, chip_id);
  stats->chUtilizationBusyRx = MTLK_DIFF_STATS(hw_stats->stats_data, hw_stats->stats_snapshot,  currentChannelStats.channelUtilizationStats[radio_idx].chUtilizationBusyRx, chip_id);
  stats->chUtilizationBusySelf = MTLK_DIFF_STATS(hw_stats->stats_data, hw_stats->stats_snapshot,  currentChannelStats.channelUtilizationStats[radio_idx].chUtilizationBusySelf, chip_id);
  stats->chUtilizationBusyExt = MTLK_DIFF_STATS(hw_stats->stats_data, hw_stats->stats_snapshot,  currentChannelStats.channelUtilizationStats[radio_idx].chUtilizationBusyExt, chip_id);
