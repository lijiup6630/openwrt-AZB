...
static void _rcvry_step_cancel_commands (wave_rcvry_card_cfg_t *pcard, uint16 oid)
{
  ...
  MTLK_UNUSED_VAR(rcvry_type);

  MTLK_ASSERT((RCVRY_TYPE_NONE == rcvry_type) ||
              (RCVRY_TYPE_UNRECOVARABLE_ERROR == rcvry_type));
...
wave_in_fw_recovery (mtlk_core_t *core)
{
  ...
  return (wave_rcvry_mac_fatal_pending_get(hw) ||
          mtlk_core_is_hw_halted(core) ||
          (RCVRY_TYPE_UNDEF != wave_rcvry_type_current_get(hw)));
}
