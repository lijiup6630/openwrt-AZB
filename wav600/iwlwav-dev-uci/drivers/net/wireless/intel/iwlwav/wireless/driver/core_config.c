...
void core_cfg_flush_four_addr_list (mtlk_core_t *nic)
{
  mtlk_hash_enum_t              e;
  MTLK_HASH_ENTRY_T(ieee_addr) *h;
  if (nic==NULL)  // enli enli +++ to fix call trace RIP in mtlk_hash_enum_first_ieee_addr() in kernel 5.4 when insert ko module
  {
    ELOG_V("If nic is NULL, it will call trace when running mtlk_hash_enum_first_ieee_addr()");
    return; 
  }
  else
  {
    ELOG_V("nic is not NULL.");
	if (&nic->four_addr_sta_list==NULL || &nic->four_addr_sta_list.hash==NULL)
	{
		ELOG_V("If nic->four_addr_sta_list is NULL, it will call trace when running mtlk_hash_enum_first_ieee_addr()!");
    	return;
	}
	else
		ELOG_V("nic->four_addr_sta_list is not NULL.");
  }  // enli enli ---
  mtlk_osal_lock_acquire(&nic->four_addr_sta_list.ieee_addr_lock);
...
_wave_core_driver_warning_detection_ind_handle (whm_warning_id warning_id, whm_warning_layer warning_layer)
{
  ...
  /* Send NL event to WHM Daemon */
  for (i = 0; i < nof_cards; i++) { // enli enli +++ to fix compiler warning: master_df may not be initiate in kernel 5.4
	hw = wave_hw_mmb_get_current_card(i);
	if (hw != NULL) {
	master_df = wave_hw_df_get(hw, 0);
	if (NULL == master_df) {
 ELOG_DD("Error Getting MasterVap warning ID : %d : card : %d not handled", warning_id, mtlk_hw_mmb_get_card_idx(hw));
  ...
  return res;
	  ...
	  } // end of "NULL == master_df" // enli enli +++
	} // end of "hw != NULL"
  }  // end of for 
return MTLK_ERR_NOT_IN_USE; // enli enli ---
}
