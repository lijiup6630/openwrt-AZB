...
#define WAVE_CSI_DATA_OFFSET      (WAVE_CSI_UDP_HEADER_SIZE + WAVE_CSI_SIGNATURE_SIZE + WAVE_CSI_SID_SIZE + WAVE_CSI_VAP_ID_SIZE)
...
wave_core_check_csi_frag (mtlk_core_t *nic, uint8 *csi_data)
{
  ...
  if (csi_signature == WAVE_CSI_SIGNATURE) {
	...
	ILOG1_D("CSI DEBUG : vap_id recv in CSI data : %u", vap_id);

#ifdef MTLK_LGM_PLATFORM
{     /* In LGM with ZWDFS enable, DCDP does not support CDB, so there is no translation of VAPID involved in DCDP */
      /* So, Convert absolute vapid to relative vapid */
      wave_radio_t *radio = wave_vap_radio_get(nic->vap_handle);
      if (wave_radio_is_5g_in_zwdfs_cdb_cfg(radio)) {
        uint8 max_vaps_count = mtlk_vap_manager_get_max_vaps_count(mtlk_vap_get_manager(nic->vap_handle));
        vap_id = vap_id & (max_vaps_count - 1);
      }
}
#endif
    ...
    if (nic->csi_more_frag != 0 && nic->csi_data_len != 0) {
      ...
      ELOG_DDD("Received 1st CSI fragment for SID %d while already having incomplete fragments for previous CSI sample for SID %d, Previous CSI sample for SID %d will be discarded", sid, nic->csi_sid, nic->csi_sid);
      if (nic->csi_stats)
...
wave_core_copy_csi_stats (mtlk_core_t *master_nic, uint8 *csi_data, uint32 data_len, uint8 is_first_frag)
{
  ...
  if (_chipid_is_gen6_d2(chip_id)) {
    full_csi_data_len = WAVE_CSI_DATA_LEN_D2;
  } else {
    full_csi_data_len = WAVE_CSI_DATA_LEN_B0;
  }
  ...
  wave_memcpy((master_nic->csi_stats + master_nic->csi_data_len), ((sizeof(wifi_csi_driver_data_hw_t) + WAVE_CSI_AGGR_PAD) - master_nic->csi_data_len), csi_data, data_len);

  master_nic->csi_data_len += data_len;
  if (master_nic->csi_data_len < full_csi_data_len)
    master_nic->csi_more_frag = 1;

  ILOG1_DD("CSI : DATA LEN accumulated : %d for sid : %d", master_nic->csi_data_len, master_nic->csi_sid);
  /* Complete CSI sample received - No more fragment expected */
  if (master_nic->csi_data_len >= full_csi_data_len) {

    core = wave_get_core_from_vap_id (master_nic);
    if(core) {
      csi_sta = mtlk_stadb_find_sid (mtlk_core_get_stadb(core), master_nic->csi_sid);
      if (csi_sta != NULL) {
        ILOG1_D("CSI - Complete data received for SID : %d", master_nic->csi_sid);
        mtlk_osal_lock_acquire(&csi_sta->lock);
        wave_memcpy(&csi_sta->csi_stats, sizeof(wifi_csi_driver_data_hw_t), master_nic->csi_stats, sizeof(wifi_csi_driver_data_hw_t));
        mtlk_osal_lock_release(&csi_sta->lock);
        csi_sta->csi_sta_cntrs.csiRecvFrameCount++;
        /* send the NL event with CSI stats here */
        _mtlk_df_user_invoke_core_async(mtlk_vap_get_df(core->vap_handle), WAVE_CORE_SEND_CSI_STATS_EVENT, &master_nic->csi_sid, sizeof(master_nic->csi_sid), NULL, 0);
        mtlk_sta_decref(csi_sta); /* De-reference of find */
      } else {
        ELOG_DDD("CID:%04x Error in getting sta entry for copying csi stats - sid : %d : vap_id : %d", mtlk_vap_get_oid(core->vap_handle), master_nic->csi_sid, master_nic->csi_vap_id);
      }
    } else {
      ELOG_D("CSI-Error VAP not found - vap_id: %d", master_nic->csi_vap_id);
    }
    master_nic->csi_more_frag = 0;
    master_nic->csi_data_len = 0;
...
wave_update_psd_paramdb (mtlk_core_t *core, u8 bss_flags)
{
  ...
  if (MTLK_BFIELD_GET(bss_flags, VAP_ADD_FLAGS_HE)) {
