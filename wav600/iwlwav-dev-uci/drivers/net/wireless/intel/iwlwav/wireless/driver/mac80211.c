...
static int _wv_request_get_sta_info (mtlk_df_user_t *df_user, st_info_data_t *info_data)
{
  mtlk_clpb_t *clpb = NULL;
  mtlk_error_t res = MTLK_ERR_OK;

  if (wave_in_fw_recovery(mtlk_vap_get_core(mtlk_df_get_vap_handle(mtlk_df_user_get_df(df_user)))))
    ...
    struct station_info *sinfo)
{
  ...
  if (wave_in_fw_recovery(mtlk_vap_get_core(vap_handle))){
    return;
  }
  ...
  if (MTLK_ERR_OK != _wv_request_get_sta_info(df_user, &info_data)) {
    WLOG_V("Getting sta info for statistics failed");
    return;
  }
  ...
  MTLK_CLPB_TRY(sta_stats, stats_size)
    ...
    priv_sta = wv_ieee80211_get_sta(sta);
    if (MTLK_BFIELD_GET(priv_sta->info.flags, STA_FLAGS_11ax)) {
      sinfo->txrate.flags |= RATE_INFO_FLAGS_HE_MCS;
      sinfo->txrate.mcs = mtlk_sta_get_max_mcs(priv_sta);
      sinfo->txrate.nss = mtlk_sta_get_max_antennas(priv_sta);
    } else if (MTLK_BFIELD_GET(priv_sta->info.flags, STA_FLAGS_11ac)) {
      sinfo->txrate.flags |= RATE_INFO_FLAGS_VHT_MCS;
      sinfo->txrate.mcs = mtlk_sta_get_max_mcs(priv_sta);
      sinfo->txrate.nss = mtlk_sta_get_max_antennas(priv_sta);
    } else if (MTLK_BFIELD_GET(priv_sta->info.flags, STA_FLAGS_11n)) {
      sinfo->txrate.flags |= RATE_INFO_FLAGS_MCS;
      sinfo->txrate.mcs = mtlk_sta_get_max_mcs(priv_sta);
    } else /* for legacy */
      sinfo->txrate.legacy = mtlk_sta_get_bitrate_max(priv_sta);
    bw = mtlk_sta_get_max_bw(priv_sta);
    if (bw == IEEE80211_STA_RX_BW_20)
      sinfo->txrate.bw = RATE_INFO_BW_20;
    else if (bw == IEEE80211_STA_RX_BW_40)
      sinfo->txrate.bw = RATE_INFO_BW_40;
    else if (bw == IEEE80211_STA_RX_BW_80)
      sinfo->txrate.bw = RATE_INFO_BW_80;
    else if (bw == IEEE80211_STA_RX_BW_160)
      sinfo->txrate.bw = RATE_INFO_BW_160;
    if (mtlk_sta_get_short_gi(priv_sta))
      sinfo->txrate.flags |= RATE_INFO_FLAGS_SHORT_GI;
    mtlk_sta_decref(priv_sta);
