 ...
 *
 *  \return     Linux error code
 * enli enli add ".policy = VENDOR_CMD_RAW_DATA" in VENDOR_CMD_WDEV & VENDOR_CMD_WDEV_UP 
 */
static int
                                        ...
                                        int data_len)
{
  ...
  if (nla_put(rsp_nbuf, NL80211_ATTR_VENDOR_DATA, data_len, data)) {
    ELOG_D("Not sufficient space to put data (%d bytes) to NL buffer", data_len);
    kfree_skb(rsp_nbuf);
    return -EMSGSIZE;
  }
