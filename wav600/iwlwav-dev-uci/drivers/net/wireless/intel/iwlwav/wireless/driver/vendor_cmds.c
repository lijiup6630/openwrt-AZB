 ...
 *
 */
static int _wave_ieee80211_vendor_get_LaMuOfdmaStats(struct wiphy *wiphy, struct wireless_dev *wdev,
                                                     const void *data, int data_len)
{
  ILOG1_SSD("%s: Invoked from %s (%i)", wdev->netdev->name, current->comm, current->pid);
  return wave_radio_get_MuOFDMA_stats(wiphy, wdev->netdev);
}
...
/* IWLWAV statistics */
  ...
  /*OFDMA counter statistics */
  VENDOR_CMD_WDEV(GET_LINK_ADAPT_MU_OFDMA_STATS, _wave_ieee80211_vendor_get_LaMuOfdmaStats),
