    ...
    if (mtlk_osal_eth_is_multicast(ether_header->h_dest)) {
      ...
      if (mtlk_df_user_is_secondary(df_user)) {
        ...
        if (dst_sta)
          ...
          return MTLK_ERR_UNKNOWN;
      } else if (MESH_MODE_BACKHAUL_AP == MTLK_CORE_HOT_PATH_PDB_GET_INT(core, CORE_DB_CORE_MESH_MODE)) {
                         ...
                         const void* data, uint32 data_size)
{
  ...
  MTLK_CLPB_TRY(mbss_cfg, mbss_cfg_size)
    ...
    if (wave_in_fw_recovery(core))
      ELOG_V("Recovery is running, but VAP removal must be done");
...
#define _WAVE_CORE_REQ_MAP_END(_id_)                                                    \
  ...
  _WAVE_CORE_REQ_MAP_START(id)
    ...
    _MTLK_CORE_HANDLE_REQ_SERIALIZABLE(WAVE_CORE_REQ_GET_LAMUOFDMA_STATS,       wave_core_get_link_adapt_muofdma_statistics);
    ...
    _MTLK_CORE_HANDLE_REQ_SERIALIZABLE(WAVE_CORE_SEND_CSI_STATS_EVENT,          wave_core_send_csi_stats);
...
static int _core_sync_done (mtlk_handle_t hcore, const void* data, uint32 data_size)
{
  ...
  mtlk_osal_timer_cancel_sync(&core->slow_ctx->sync_hostapd_timer);
  wave_core_sync_hostapd_done_set(core, TRUE);
  ILOG0_D("CID-%04x: Recovery: Set HOSTAPD STA database synced to TRUE", mtlk_vap_get_oid(core->vap_handle));
  res = MTLK_ERR_OK;
