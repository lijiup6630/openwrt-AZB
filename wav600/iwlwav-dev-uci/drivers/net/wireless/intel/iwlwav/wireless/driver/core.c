...
_mtlk_core_crypto_cleanup (mtlk_core_t *core)
{
  ...
  if(slow_ctx->wep_rx){
    kzfree(slow_ctx->wep_rx); // enli enli crypto_free_cipher(slow_ctx->wep_rx);
  }

  if(slow_ctx->wep_tx){
    kzfree(slow_ctx->wep_tx); // enli enli crypto_free_cipher(slow_ctx->wep_tx);
  }
...
_mtlk_core_crypto_init (mtlk_core_t *core)
{
  ...
  slow_ctx->wep_rx = kmalloc(sizeof(slow_ctx->wep_rx), GFP_KERNEL); // enli enli crypto_alloc_cipher("arc4", 0, CRYPTO_ALG_ASYNC);
  if (!slow_ctx->wep_rx) {//if(IS_ERR_OR_NULL(slow_ctx->wep_rx)){
  ...
  slow_ctx->wep_tx = kmalloc(sizeof(slow_ctx->wep_tx), GFP_KERNEL);//enli enli crypto_alloc_cipher("arc4", 0, CRYPTO_ALG_ASYNC);
  if (!slow_ctx->wep_tx) {// if(IS_ERR_OR_NULL(slow_ctx->wep_tx)){
