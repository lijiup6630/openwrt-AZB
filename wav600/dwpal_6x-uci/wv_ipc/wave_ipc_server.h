...
/* Internal reference to user-defined data */
struct _wv_ipstation_data {
	void *data;
};

static inline void wave_ipcs_sta_set_data(wv_ipstation *ipsta, void *data)
{
	if (ipsta) ((struct _wv_ipstation_data*)ipsta)->data = data;
}

static inline void* wave_ipcs_sta_get_data(wv_ipstation *ipsta)
{
	return ipsta ? ((struct _wv_ipstation_data*)ipsta)->data : NULL;
}
