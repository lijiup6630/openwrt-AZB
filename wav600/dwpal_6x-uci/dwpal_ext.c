...
static volatile int listenerThreadShouldStop = 0;
static int listenerThreadPipeFds[2] = { 0 };
...
static const size_t numOfServices = sizeof(dwpalService) / sizeof(DwpalService *);

#define MUTEX_LOCK(__mutex) \
do { \
	if (pthread_mutex_lock(__mutex)) { \
		/* console_printf("%s; mutex lock error!\n", __FUNCTION__); */ \
	} \
} while(0)
...
do { \
	if (pthread_mutex_unlock(__mutex)) { \
		/* console_printf("%s; mutex unlock error!\n", __FUNCTION__); */ \
	} \
} while(0)

static DWPAL_Ret interfaceIndexGet(const char *interfaceType, const char *VAPName, int *idx)
{
	unsigned i;
	*idx = 0;
...
static DWPAL_Ret interfaceIndexCreate(const char *interfaceType, const char *VAPName, int *idx)
{
	unsigned i;
			...
			if (!strncmp(dwpalService[i]->interfaceType, "hostap", sizeof("hostap") - 1))
			{
				if (dwpalService[i]->fd > 0)
				{
					if (FD_ISSET(dwpalService[i]->fd, &rfds))
					{
						...
						// listenerThreadShouldStop may be changed during select() call. Don't call event handler if thread is cancelling
						if (!listenerThreadShouldStop)
						{
...
static DWPAL_Ret listenerThreadCreate(pthread_t *thread_id, ThreadEntryFunc threadEntryFunc)
{
	...
	if (fcntl(listenerThreadPipeFds[0], F_SETFL, O_NONBLOCK) == -1)
	{
		...
		case THREAD_CANCEL:
			if (*thread_id != 0)
			{
				...
				/* tell the listener Thread to stop, both with flag, and message.
				   While listenerThreadShouldStop is set to 1, no new "hostap_send" operations are available
				 */
				...
				/* Listener Thread has been stopped, so flag can be set back to 0 */
				listenerThreadShouldStop = 0;
				*thread_id = 0;
							   ...
							   unsigned char *outData)
{
	...
	if (dwpal_ext_interfaceIndexGet("Driver", "ALL", &idx) == DWPAL_INTERFACE_IS_DOWN)
	{
...
DWPAL_Ret dwpal_ext_interfaceIndexGet(const char *interfaceType, const char *VAPName, int *idx)
{
	DWPAL_Ret ret;

	// pthread_mutex_lock(&attach_mutex);
	ret = interfaceIndexGet(interfaceType, VAPName, idx);
...
DWPAL_Ret dwpal_ext_hostap_cmd_send(const char *VAPName, const char *cmdHeader, FieldsToCmdParse *fieldsToCmdParse, char *reply /*OUT*/, size_t *replyLen /*IN/OUT*/)
{
	...
	if (pthread_self() == listenerThreadId && listenerThreadShouldStop)
	{
	...
	if (dwpal_ext_interfaceIndexGet("hostap", VAPName, &idx) == DWPAL_INTERFACE_IS_DOWN)
	{
...
#else
	return (pthread_self() == listenerThreadId);
#endif
}
