...
/* The link_adapt_muru_ofdma_stats_t should be aligned with */
/* the driver wave_wssa_ofdma_mu_ru_stats_t structure */
typedef struct link_adapt_muru_ofdma_stats {
	uint32_t pkt_err_ru_percent;
	uint32_t ru_percent;
	uint64_t success_ru_ampdus;
	uint64_t failed_ru_ampdus;
	uint64_t total_tx_ru_ampdus;
} link_adapt_muru_ofdma_stats_t;

/* The link_adapt_sumuru_ofdma_stats_t should be aligned with */
/* the driver wave_wssa_ofdma_su_mu_ru_stats_t structure */
typedef struct link_adapt_sumuru_ofdma_stats {
	uint16_t sta_sid;
	IEEE_ADDR addr;
	uint64_t total_tx_mu_ampdus;
	link_adapt_muru_ofdma_stats_t muru[MAX_NUM_RU_SIZES];
} link_adapt_sumuru_ofdma_stats_t;

/* The link_adapt_total_ofdma_stats_t should be aligned with */
/* the driver wave_wssa_ofdma_total_stats_t structure */
typedef struct link_adapt_total_ofdma_stats {
	uint64_t all_sta_success_su_ampdus;
	uint64_t all_sta_failed_su_ampdus;
	uint64_t all_sta_success_ru_ampdus[MAX_NUM_RU_SIZES];
	uint64_t all_sta_failed_ru_ampdus[MAX_NUM_RU_SIZES];
} link_adapt_total_ofdma_stats_t;

/* The struct link_adapt_su_mu_ofdma_stats_t should match with */
/* the driver wave_wssa_ofdma_su_mu_stats struct */
typedef struct link_adapt_su_mu_ofdma_stats {
	uint16_t sta_sid;
	IEEE_ADDR addr;
	uint32_t su_pkt_err_percent;
	uint32_t mu_pkt_err_percent;
	uint32_t total_su_pkts_percent;
	uint32_t total_mu_pkts_percent;
	uint32_t total_pkt_err_percent;
	uint64_t success_su_pkts;
	uint64_t failed_su_pkts;
	uint64_t success_mu_pkts;
	uint64_t failed_mu_pkts;
	uint64_t total_tx_pkts;
	uint64_t total_fail_pkts;
} link_adapt_su_mu_ofdma_stats_t;

/* CSI Stats */
#define MAX_NR_D2            5
#define MAX_NC_D2            1
#define MAX_SUB_CARRIERS     48
#define CSI_RAW_DATA_SIZE    788
#define CSI_MATRIX_DATA_SIZE 240

/* SU_MU_RU_OFDMA stats */
#define GET_MU_RU_OFDMA_CATEGORY 0
#define GET_SU_MU_OFDMA_CATEGORY 1
#define GET_TOTAL_OFDMA_CATEGORY 0x0F

typedef signed short wifi_streams_rssi_t [MAX_NR_D2];
...
typedef enum {
	...
	PEER_LIST,
	RESET_STATISTICS,
	LINK_ADAPT_MU_OFDMA_STATS,
	LINK_ADAPT_SU_MU_RU_OFDMA_STATS,
	CSI_STATS,
	LINK_ADAPT_SU_MU_OFDMA_STATS,
}stat_id;
...
{"lamuofdmacounters", LTQ_NL80211_VENDOR_SUBCMD_GET_LINK_ADAPT_MU_OFDMA_STATS, 1,
		"usage: dwpal_cli [INTERFACENAME] LaMuOfdmaCounters", LINK_ADAPT_MU_OFDMA_STATS},
/* CSI stats */
...
/* OFDMA SuMuRu stats */
{"mudlofdmaru", LTQ_NL80211_VENDOR_SUBCMD_GET_LINK_ADAPT_SU_MU_RU_OFDMA_STATS, 1,
		"usage: dwpal_cli [INTERFACENAME] MuDLOfdmaRu", LINK_ADAPT_SU_MU_RU_OFDMA_STATS},
{"sumustats", LTQ_NL80211_VENDOR_SUBCMD_GET_LINK_ADAPT_SU_MU_RU_OFDMA_STATS, 1,
		"usage: dwpal_cli [INTERFACENAME] SuMuStats", LINK_ADAPT_SU_MU_OFDMA_STATS},
};
