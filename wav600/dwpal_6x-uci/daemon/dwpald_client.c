...
static inline int call_hostap_event(
	dwpald_hostap_attachment* hap_attach, dwpald_hostap_event_with_id* event,
	char *ifname, char *op_code, char *msg, size_t len)
{
	if (event) {
		...
		if (hap_attach->common_event) {
			hap_attach->common_event(ifname, op_code, msg, len);
		}
...
/* Send 'HOSTAP ATTACH' request to the server. Not locked with mutex */
...
static int dwpald_receive_hostap_event(wv_ipc_msg *event, dwpald_header *hdr)
{
	...
	list_foreach_start(dwpald_conn->hostap_attachments, hap_attch, dwpald_hostap_attachment)
		...
		{
			if (!strncmp(op_code, "INTERFACE_RECONNECTED_OK", sizeof("INTERFACE_RECONNECTED_OK") - 1))
			...
			list_foreach_start(hap_attch->hostap_events, hap_event, dwpald_hostap_event_with_id)
				if (!strncmp(hap_event->op_code, op_code, sizeof(op_code))) {
					call_hostap_event(hap_attch, hap_event,
						ifname, op_code, event_msg, event_msg_len);
					found = 1;
					break;
				}
			list_foreach_end
...
err:
	...
	if (hap_attachment) {
		if (hap_attachment->hostap_events)
			list_free(hap_attachment->hostap_events);
		free(hap_attachment);
	}
	return ret;
}
