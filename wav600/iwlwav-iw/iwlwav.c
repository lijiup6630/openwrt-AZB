...
/***************************** GET FUNCTIONS *****************************/
static int get_intel_attr_vendor_data (struct nlattr **attr)
{
	/* Driver puts NL80211_ATTR_VENDOR_DATA 'struct nlattr' twice */
	if (nla_len(*attr) <= NLA_HDRLEN)
		return 1;
...
static int print_probe_req_list(struct nl_msg *msg, void *arg)
{
	...
	if (get_intel_attr_vendor_data(&attr))
		return NL_SKIP;
